...
Effort_GN_TN <-  all_GN_TN %>%
  select(SURVEY_ID, SURVEYDATE, SURVEYTYPE, DOW, LKNAME, YEAR, GEAR, EFFORT)%>% #################### could consider dumping the Survey_ID here
  group_by(SURVEY_ID, SURVEYDATE, SURVEYTYPE, DOW, LKNAME, YEAR, GEAR, EFFORT)%>% #################### could consider dumping the Survey_ID here
  summarise(NUMBER_OF_SP = n())%>%
  select(-NUMBER_OF_SP)%>%
  filter(SURVEYTYPE %in% c("Population Assessment", "Re-Survey", "Standard Survey", "Special Assessment", "Targeted Survey", "Large Lake Survey", "Research Survey", "Initial Survey"))%>% ######### these survey types are not currently excluded from the data product you send to chris, below
...
MN_Catch_0 <- all_GN_TN %>%
  mutate(DOW = str_pad(DOW, 8, side = "left", pad = "0"))%>%  ############### pads Kittle Numbers for stream and river surveys (Not REALLY a problem, but weird)
  ...
  # select(!contains("frog"))%>% #attempt to drop non-fish species  ############# Not really a problem, but you don't have to get rid of weird shit at this phase  ####################
