...
#new hive
```{r}
mw_data <- open_dataset(sources = file.path("D:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Fish Survey Data", "Parquet files", "hive_in_update"), partitioning = c("state")) 
glimpse(mw_data)
...
mw_data %>% 
  distinct(state, lake_id, lake_name, nhdhr_id, lat_unspec) %>% 
  group_by(state, is.na(nhdhr_id), is.na(lat_unspec)) %>% 
  count() %>% 
  collect()
