...
#now add in no fish efforts and make nothing caught columns 
...
ll_fish_effort %>% 
  group_by(is.na(SP),
           is.na(flag)) %>% 
  count()
#bomb - we can now see that we have added 687 rows of nothing caught nets 
#we also added one column (the flag column)
...
ll_fish_effort %>% 
  filter(str_detect(flag, "misleading")) %>% 
  select(flag)
...
ll_fish_effort<- ll_fish_effort %>% 
  ...
  mutate(sub_effort_nothing_caught = is.na(species_1)) %>% 
  ungroup() %>% 
  group_by(total_effort_ident) %>% 
  mutate(total_effort_nothing_caught = all(sub_effort_nothing_caught)) %>% 
  ungroup()
...
aging_check <- ll_fish_effort %>% 
  distinct(NAME, Year, sub_effort_ident, LEN_MM, WT_G, species_1, OFF_AGE, AGE_SRC, CALCIFIED_STRUC_TYPE_NAME, SERIAL) %>% 
  arrange(sub_effort_ident, LEN_MM, WT_G, OFF_AGE, SERIAL)
#when you arrange by net, length, weight, species, serial - it is pretty clean the serial number is for identifying individual fish

aging_check %>% 
  group_by(sub_effort_ident, LEN_MM, WT_G, species_1, OFF_AGE, AGE_SRC, CALCIFIED_STRUC_TYPE_NAME) %>% 
  count()
...
check <- aging_check %>% 
  filter(sub_effort_ident == "100001866")
rm(check)
...
check <- aging_check %>% 
  ...
  group_by(sub_effort_ident, species_1, SERIAL) %>% 
  filter(n_distinct(LEN_MM) > 1) %>% 
  ungroup() %>% 
  arrange(sub_effort_ident, species_1, SERIAL, LEN_MM)
# looks like most could be be the same fish just measured twice with measurement error
...
check <- ll_fish_effort %>% 
  ...
  select(sub_effort_ident, NAME, Year, SERIAL, species_1, LEN_MM, WT_G, AGE_SRC, OFF_AGE) %>% 
  group_by(sub_effort_ident, species_1, LEN_MM, WT_G) %>% 
  count() %>% 
  filter(n > 1)
...
check<- ll_fish_effort %>% 
  ...
  select(sub_effort_ident, NAME, Year, SERIAL, species_1, LEN_MM, WT_G, AGE_SRC, OFF_AGE) %>% 
  arrange(sub_effort_ident, species_1, LEN_MM, WT_G) 
...
ll_fish_effort <- ll_fish_effort %>% 
  ...
  group_by(sub_effort_ident, fish_id) %>% 
...
ll_fish_effort %>% 
  ...
  mutate(total_obs = n * nn,
         remaining_fish = nn)
#the sum of the "remaining fish" matches to the number of records from the slice above which is perfect
#however, it is valid that a fish might be aged 3+ times... or 17 times?
...
ll_fish_effort %>% 
  filter(sub_effort_ident == "67056900" & SERIAL == "10029") %>% 
  glimpse()
...
########################update from 3/7###################################
everything is jamming well - I still need to update there columns:
1. Flag
2. Original_file_names
3. notes_1 - includes mesh size and maturity 
```{r}
...
pep_fish_effort <- pep_fish %>% 
  left_join(total_effort)

pep_fish_effort %>% 
  filter(is.na(Number_Nets)) %>% 
  group_by(year) %>% 
  count()
#call fish records have effort - sweet
