...
## To Do List
    ...
    2. Aged fish data need to be plugged in (after completing the organization & join of the multiple age files, BUT we think that we're getting a new age file)
    3. Total effort 2 ("distance") did not come to us with any units on it
    4. lengths from binned fish need to be 1) moved over to length col, 2) units rectified (can do this using word and str_detect see code examples in this doc. then split rnage into min_length, max_length and convert those numerics, then re-paste those two cols into a single one. )
    5. Review fishcounts by species, currently very wonky numbers for many species (ia_data %>% group_by(species.1) %>% summarise(n = n()) %>% collect())
    
## Questions for Data Provider:
- why a bunch of records w/o any data: 
    #why some with no sample ID? Doesn't matter-- we have no effort data for these, and no species info
...
- Confirm my understanding of the bad total_count = 0 records was correct
- Check on the units of effort
...
# The zeros that are in here are very limited and I'd suggest they are wonky and inconsistent. 
...
# ia_cpue_21Aug2021[ , .N , .( date.1, lake_id,  sampling_method.1) ]
...
# ia_cpue_21Aug2021[ , .N , .( date.1, lake_id,  sampling_method.1) ][!(ia_fishlengths_24Oct2023[ , .N , .(date.1, lake_id,  sampling_method.1)]), on = .( date.1, lake_id,  sampling_method.1) , ]


##Back to the fish data:
ia_fishlengths_24Oct2023[ , .N , .(date.1, date_clean, lake_id,  sampling_method.1)] #check the date cleaning
...
cols <- as.character(expression(
    state, county, lake_id, lake_name.1, date_clean, date.1, survey_id,
    site_id.1, sampling_method.1, total_effort_1, distance, 
...
ia_fishlengths[ , length(unique(sampling_method.1, survey_id)) , site_id.1][V1>1] #sure looks like it based on this!!!
ia_fishlengths[ , .N , .(site_id.1, sampling_method.1, survey_id, total_effort_1) ] # and we seem to have ONE effort value for each of these! What a miracle!
  setnames(ia_fishlengths, "site_id.1", "total_effort_ident")
  ia_fishlengths[ , total_effort_1_units := "hours"]
  ia_fishlengths[ , .N , .(total_effort_ident, total_effort_1, distance, sampling_method.1, total_effort_1_units) ]
  ia_fishlengths[ , summary(distance)]
  setnames(ia_fishlengths, "distance" , "total_effort_2")
  ia_fishlengths[ , total_effort_2_units := "distance, units unknown"]
  ia_fishlengths[ , .N , .(total_effort_ident, total_effort_1, total_effort_1_units, total_effort_2, total_effort_2_units, sampling_method.1) ]
  
#sampling method
ia_fishlengths[ , .N , sampling_method.1 ]


#length and weight
ia_fishlengths[ , `:=` (length_unit.1 = "centimeter", weight_unit.1 = "grams") , ]
  ia_fishlengths[species.1 == "Walleye", summary(length.1)]
  
#species names
  ia_fishlengths[ , .N , species.1]
  ia_fishlengths[ , .N , species.1][,species.1]
    make_clean_names(ia_fishlengths[ , .N , species.1][,species.1])
  ia_fishlengths[ , species.1 := as.factor(species.1) ,] 
  setattr(ia_fishlengths$species.1, "levels", make_clean_names(ia_fishlengths[ , .N , species.1][,species.1]))
  

#sample biological info
ia_fishlengths[ ,.N , .( is.na(sex)) ]
ia_fishlengths[ ,.N , .( is.na(aging_structure.1)) ]  
ia_fishlengths[ ,.N , .( is.na(aging_structure.2)) ]  
ia_fishlengths[ ,.N , .( is.na(aging_structure.3)) ]#trash

ia_fishlengths[ ,.N , .( is.na(aging_data_notes.1)) ]#trash  
ia_fishlengths[ ,.N , .( is.na(aging_data_notes.2)) ]#trash  
ia_fishlengths[ ,.N , .( is.na(aging_data_notes.3)) ]#trash


#sample quality notes
ia_fishlengths[ ,.N , .(garbage_bin_notes.4)]
ia_fishlengths[ , .N , garbage_bin_notes.5]
ia_fishlengths[ , .N , garbage_bin_notes.6 ]

ia_fishlengths[garbage_bin_notes.4 == "IsSampleQualityAdequate:FALSE" |
                garbage_bin_notes.5 == "SamplingIssue:TRUE" ,
               flag := "sample_issue_state_flagged", ]

#batch counted fish sometimes have lengths (for now we simply move those data over to their own column. In future we'll actually munge them into the length col)
ia_fishlengths[ , .N , garbage_bin_notes.13]
  ia_fishlengths[ , batch_l := word(garbage_bin_notes.13, 2, sep = fixed(":")) ,]
  ia_fishlengths[batch_l == "NA" , batch_l := NA , ]
  ia_fishlengths[ !is.na(batch_l), .N , .(batch_l,species.1) ]
  ia_fishlengths[ !is.na(batch_l), .N , .(batch_l) ]
    ia_fishlengths[ str_detect(batch_l, ".99"), .N , .(batch_l, length.1, garbage_bin_notes.14, garbage_bin_notes.13) ]
    ia_fishlengths[ str_detect(batch_l, ".99"), .N , .(batch_l, length.1, garbage_bin_notes.14, garbage_bin_notes.13) ]
  #convert length to chr to recieve these:
  # ia_fishlengths[  , length.1:= as.character(length.1)  ,  ]
  # # now move bin data over there
  # ia_fishlengths[str_detect(batch_l, ".99")  , `:=` (length.1 = word(batch_l, 1, sep = fixed(".")),
  #                                                    length_unit.1 = "centimeter",
  #                                                    batch_l = NA), ]
  # ia_fishlengths[ str_detect(batch_l, "mm"), .N , .(batch_l, length.1, garbage_bin_notes.14, garbage_bin_notes.13) ]
  #   # now move bin data over there
  #   ia_fishlengths[str_detect(batch_l, "150mm")  , `:=` (length.1 = word(batch_l, 1, sep = fixed(".")),
  #                                                      length_unit.1 = "centimeter",
  #                                                      batch_l = NA), ]
  # 
  
#nothing caught (not yet relevant)
    ia_fishlengths[ , .N , total_effort_ident][order(N)]
  
    

cols <- as.character(expression(
    state, county, lake_id, lake_name.1, date_clean, date.1, survey_id,
    total_effort_ident, total_effort_1, total_effort_1_units, total_effort_2, total_effort_2_units, sampling_method.1,
    sample_id.1, species.1, length.1, length_unit.1, weight.1, weight_unit.1, sex, batch_l,
    aging_structure.1, aging_structure.2,
    start_time,
    lake_type, location_notes.1, lat_unspec, lon_unspec, 
    original_file_name.1,
    flag))

ia_fishlengths <- ia_fishlengths[ , .SD  , .SDcols = cols ]
#notes on this file: 1) has the binned fish lengths in batch_l column; 2) contains no effort units where nothing was caught; contains no ages for these fish because we're waiting on a new database rip from IA
...
#do these data contain any no_fish_caught surveys?
ia_fishobs_30Nov2023[total_count.1 == 0, .N] #Coool. 12k zeros!

## Data Identifiers
#Here in these data, the *survey_id* is equivalent to our total_effort_ident. 
ia_fishlengths_24Oct2023[ , .N , survey_id][ , summary(N)] #every survey has at least one line
ia_fishlengths_24Oct2023[ , .("countfish" = sum(total_count.1)), survey_id][ , summary(countfish) ,] #none of the surveys show no fish caught
...
# The had a var called survey.SurveyID that we renamed to Secondary_Lake_ID -- this combines....???????????
ia_fishobs_30Nov2023[ , length(unique(secondary_lake_id)) , .(lake_name.1, date.1, survey_type.1 ) ][V1>1]

a <- ia_fishobs_30Nov2023[lake_name.1 == "Ada Hayden Heritage Park Lake" & date.1 == "10/4/2010"  & survey_type.1 == "General" ]
  a[ , length(unique(secondary_lake_id)) , .(lake_name.1, date.1, survey_type.1 ) ][V1>1]
...
ia_fishobs[ , length(unique(total_effort_1)) , sub_effort_ident ]
...
#do we have any sub_effort_idents where the batch_count is the holder of that gear?
  ia_fishobs_30Nov2023[ , .() , sub_effort_ident  ]
  


#consider: notes.8/fish.measurement.id changed so NA if no fish measure on that line (currently == "")
ia_fishobs_30Nov2023[ species.1 == "", unique(notes.8) , ]





#batch count idents & counts:
ia_fishobs_30Nov2023[ , unique(batch_count_ident) , ]
  ia_fishobs_30Nov2023[, .N , .(total_count.1,individual_count, batch_count_ident) ]
ia_fishobs_30Nov2023[ batch_count_ident == "", .N , batch_length ]
ia_fishobs_30Nov2023[ batch_count_ident == "", c("batch_count_ident", "batch_length", "batch_weight") := NA , ]
  ia_fishobs_30Nov2023[ !is.na(batch_count_ident) , .(total_count.1, individual_count) , .(batch_count_ident) ][!is.na(total_count.1), , ] #looky there, now no cases of total count not NA in the batch measured fish. 
  ia_fishobs_30Nov2023[is.na(batch_count_ident) & individual_count >1, .N , ] #any of the non-batch count data tagged as indiv count>1?  
#total_count is a count of all fish in a survey_id?
ia_fishobs_30Nov2023[is.na(batch_count_ident), .N , total_count.1]
ia_fishobs_30Nov2023[, length(unique(total_count.1)), sample_id.1 ][V1>1]
  a <- ia_fishobs_30Nov2023[sample_id.1 == "8D08DB00-389D-4B90-804E-B07CD6B530AC" ]
...
ia_catch_eff <- as_arrow_table(ia_fishlengths)
