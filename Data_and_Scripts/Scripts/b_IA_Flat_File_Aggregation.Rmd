...
files_list <- list.files(path = "G:/Shared drives/Hansen Lab/RESEARCH PROJECTS/Fish Survey Data/IA_Data/ia_raw_disaggregated_data", pattern = ".+\\.csv") #grabs only.csv files
files_list
...
for(i in 1:n) {
  ...
  assign(filei ,
          fread(paste0("G:/Shared drives/Hansen Lab/RESEARCH PROJECTS/Fish Survey Data/IA_Data/ia_raw_disaggregated_data/",
                                          files_list[i])))
...
#Obs Data
-this chunk of code uses the fish observation data to:
1. add total effort and sub effort nothing caught columns
2. uncounts batch counted fish
3. creates a total effort nothing caught column
4. adds gear corrections 
```{r}
...
# because they drew together databases, there's a bit of craziness that happened with the survey_ident having 1 record that  says total_count = 0 (indicating that total_eff_ident has 0 indiv_fish observations), but one batch count record with N>0
...
#Now we have improved gear names, generate new total effort idents:
      ...
      ###########creation of the total effort ident grouping
...
-this chunk keeps columns we want and throws out junk
-adds flags
```{r}
...
#I will then take consolidated gear as sampling method 2 - more refined gear type
rm(sampling_methods)
...
ia_dat_old <-  fread("G:\\Shared drives\\Hansen Lab\\RESEARCH PROJECTS\\Fish Survey Data\\IA_Data\\ia_raw_disaggregated_data\\Archived\\ia_fishlengths_24Oct2023.csv")
...
#any places where the diff of lat/lon is alarming? - nope!
rm(check)

#Redenius Pond - RED24 - has 0 for iowa reported lat and lon with NA info from the other locations, lets remove that
ia_fishobs <- ia_fishobs %>% 
  mutate(lat_unspec = case_when(lat_unspec == 0 ~ NA,
                                TRUE ~ lat_unspec),
         lon_unspec = case_when(lon_unspec == 0 ~ NA,
                                 TRUE ~ lon_unspec)) 
#no more 0s in those columns
ia_fishobs %>% 
  filter(lat_unspec <2 | lon_unspec > -2) %>% 
  glimpse()
ia_fishobs %>% 
  filter(lake_id == "RED24") %>% 
  distinct(lat_unspec, lon_unspec)
...
#total effort 1 NA clean up
#total effort 1 is strictly time while total effort 2 is distance 
#first i want to move records where distance is recorded but not time to total effort 1
ia_fishobs <- ia_fishobs %>% 
  mutate(total_effort_1 = case_when(is.na(total_effort_1) & !is.na(total_effort_2) ~ total_effort_2,
                                    TRUE ~ total_effort_1),
         total_effort_1_units = case_when(is.na(total_effort_1_units) & !is.na(total_effort_2_units) ~ total_effort_2_units,
                                          TRUE ~ total_effort_1_units))
ia_fishobs %>% 
  group_by(total_effort_1_units, 
           total_effort_2_units,
           is.na(total_effort_1),
           is.na(total_effort_2),
           is.na(flag)) %>% 
  count()
#we can now see there are no cases where total effort 1 is na but total effort 2 has a value
#similarly, we can see that the unit is retained with the transfer of total effort 
#however, there still remains some records that are NA for total effort without a flag

#remaining obs that have na total effort without flag
check_total <- ia_fishobs %>% 
  filter(is.na(total_effort_1) & is.na(flag)) %>% 
  distinct(total_effort_ident, lake_name_1, survey_start_date, survey_id, total_effort_1, total_effort_2, sub_effort_1, sub_effort_2, sampling_method_simple, sampling_method_1, sampling_method_2, total_effort_1_units, total_effort_2_units)

#filter to being back sub efforts even if there is a flag but total effort for collective is na
check_sub <- ia_fishobs %>% 
  filter(total_effort_ident %in% check_total$total_effort_ident) %>% 
  distinct(total_effort_ident, lake_name_1, survey_start_date, survey_id, total_effort_1, total_effort_2, sub_effort_ident, sub_effort_1, sub_effort_2, sampling_method_simple, sampling_method_1, sampling_method_2, flag)
#here we can see there are a few different reasons a total effort ident may still be na 
#there are sometimes multiple gears listed within a total effort ident or the agency did not report effort at various levels
#sometimes effort for a singular sub effort was not reported - however that NA in the sub effort leads to NA total effort during summation 

#given the small amount of na total effort surveys remaining without an "easy" integration given the wonkiness - lets remove 
#first, make a list of total effort ident to remove
ident_remove <- check_total %>% 
  distinct(total_effort_ident)

ia_fishobs <- ia_fishobs %>% 
  filter(!(total_effort_ident %in% ident_remove$total_effort_ident))
ia_fishobs %>% 
  group_by(is.na(total_effort_1), is.na(flag)) %>% 
  count()
#now we can see that total effort idents with na total effort are listed with a flag

rm(check_total, check_sub, ident_remove)

#this checking process looks like 
#1. find places that are NA for total effort but have a flag
#2. take the the survey id from that total effort and plug it into the filter below for survey id to get original data
#3. returns a df with relevent info

#based on some level of research, I can see that there was a flag created based on notes.2 column for inadequate sample
#this note is applied to all levels within the survey id/total effort ident 
#we can see that there are sub efforts within the total effort that remain unaffected but get the flag tag because they are in the same total effort 
#next exploration is trying to figure out if it is worth recooping these sub efforts 

#with flag na effort?
ia_fishobs %>% 
  mutate(na_effort = case_when(is.na(total_effort_1) ~ T,
                               T ~ F)) %>% 
  summarise(prop_na_effort_samples = mean(na_effort))
#16% of the total samples 

check <- ia_fishobs %>% 
  filter(is.na(total_effort_1)) %>% 
  distinct(total_effort_ident,survey_id, sub_effort_ident, sampling_method_1, sampling_method_2, total_effort_1, total_effort_2, sub_effort_1, sub_effort_2, flag)


#do the na efforts with flags call hold a similar patter - filter orignal ia obs and each notes.2
spec_check <- ia_fishobs_30Nov2023 %>% 
  filter(survey_id %in% c(check$survey_id)) %>% 
  distinct(lake_name.1, survey_id, date.1, sampling_method.1, sampling_method.2, total_effort_1, notes.2, total_effort_ident, sub_effort_ident)
...
spec_check %>% 
  group_by(total_effort_ident) %>% 
  summarize(has_adequate_0 = any(notes.2 == "Sample.IsSampleQualityAdequate:0")) %>% 
  summarize(all_have_adequate0 = all(has_adequate_0)) %>% 
  pull(all_have_adequate0)
#this tells us that every total_effort_ident has at least one sub effort ident that is not adequate - likely leading to NA total effort 

spec_check %>% 
  group_by(notes.2, is.na(total_effort_1)) %>% 
  count()
#we can see that the only cases that are NA for total effort are the adequate 0s which pick up a flag early in the ag process 

check %>% 
  filter(is.na(total_effort_1) & !is.na(flag)) %>% 
  distinct(total_effort_ident, sub_effort_ident, flag) %>% 
  summarise(sum_na = sum(is.na(flag))) 

check %>% 
  filter(is.na(total_effort_1) & !is.na(flag)) %>% 
  distinct(survey_id, total_effort_ident, sub_effort_ident, sub_effort_1) 

ia_fishobs %>% 
  filter(visit_id == "A971153C-0FB8-4B55-993B-90644B8926D0") %>% 
  distinct(lake_name_1, date_1, sampling_method_1, sampling_method_2, total_effort_ident, sub_effort_ident, total_effort_1, sub_effort_1, visit_id)

ia_fishobs %>% 
  mutate(across(everything(), as.character)) %>% 
  purrr::map_lgl(~ any(. == "A971153C-0FB8-4B55-993B-90644B8926D0", na.rm = T)) %>% 
  which()
  
rm(check, spec_check)
########################################################################

####are there cases of multiple total efforts idents in a year-lake-gear (grouping that makes up the ident)
multi_tei_in_year <- ia_fishobs %>% 
  group_by(lubridate::year(survey_start_date), lake_name_1, consolidated_gear) %>% 
  summarise(n_total_effort_idents = n_distinct(total_effort_ident)) %>% 
  filter(n_total_effort_idents >1)

check <- ia_fishobs %>% 
  group_by(year = lubridate::year(survey_start_date), lake_name_1, consolidated_gear) %>% 
  summarise(
    n_total_effort_idents = n_distinct(total_effort_ident),
    .groups = "drop"
  ) %>% 
  filter(n_total_effort_idents > 1) %>%
  inner_join(
    ...
    by = c("year", "lake_name_1", "consolidated_gear")
  ) %>% 
  mutate(year = lubridate::year(survey_start_date)) %>% 
  distinct(lake_name_1, 
           ...
           total_effort_ident,
           survey_start_date)
#in the first call, we find all cases where there are multiple total efforts of the same gear-lake within a year
#second call shows why that might be:
#it looks like in a lot of cases there is a large range in when sampling with the same method (fall and spring), other cases looks like the grouping gear was different than the more defined gear in the end 

check_org <- ia_fishobs %>% 
  group_by(year = lubridate::year(survey_start_date), lake_name_1, consolidated_gear) %>% 
  summarise(
    n_total_effort_idents = n_distinct(total_effort_ident),
    .groups = "drop"
  ...
  distinct(lake_name_1, 
           ...
           total_effort_ident,
           survey_start_date) %>% 
  group_by(lake_name_1, survey_start_date, consolidated_gear) %>% 
  summarise(n_total_idents = n_distinct(total_effort_ident),
            n_survey_ids = n_distinct(survey_id))

#here we can see an example of a sampling_start_dateXgearXlake that has 10 different tei, stemming from differen survey ids
specific_check <- ia_fishobs %>% 
  filter(lake_name_1 == "Pool 16, Mississippi River", consolidated_gear == "Hoop Net Unspecified" & survey_start_date == "2021-07-19") %>% 
  group_by(survey_id, total_effort_ident, sub_effort_ident, lake_name_1, consolidated_gear, survey_start_date) %>% 
  count()

#how common is this?
ia_fishobs %>% 
  group_by(consolidated_gear, survey_start_date, lake_name_1) %>% 
  summarise(n_survey_id = n_distinct(survey_id),
            n_total_effort_ident = n_distinct(total_effort_ident)) %>% 
  filter(n_survey_id > 1) %>% 
  arrange(desc(n_survey_id))
#only 241 lakeXgearXdate this happens

ia_fishobs %>% 
  group_by(consolidated_gear, survey_start_date, lake_name_1) %>% 
  summarise(n_survey_id = n_distinct(survey_id),
            n_total_effort_ident = n_distinct(total_effort_ident)) %>% 
  mutate(multi_tei = case_when(n_survey_id > 1 ~ T,
                               n_survey_id <= 1 ~ F)) %>% 
  ungroup() %>% 
  summarise(prop_samples = mean(multi_tei))

ia_fishobs %>% 
  group_by(consolidated_gear, survey_start_date, lake_name_1) %>% 
  summarise(n_survey_id = n_distinct(survey_id),
            n_total_effort_ident = n_distinct(total_effort_ident)) %>% 
  mutate(mismatch = case_when(n_survey_id != n_total_effort_ident  ~ T,
                               n_survey_id == n_total_effort_ident ~ F)) %>% 
  ungroup() %>% 
  summarise(prop_mismatch = mean(mismatch))
#this shows that the number of total efforts is alway equal to the number of survey_ids, cherry on top for evidence that this is driving the issue

rm(multi_tei_in_year, check, check_org, specific_check)
##########################################################

#sex column correction 
ia_fishobs <- ia_fishobs %>% 
  mutate(sex = case_when(sex == "" ~ NA,
                         sex == "Undetermined" ~ "Unknown",
                         TRUE ~ sex)) 

#cleaning up age class
ia_fishobs <- ia_fishobs %>% 
  mutate(age_class = case_when(age_class == "Adult" ~ "adult",
                               age_class == "Sub-Adult" ~ "sub_adult",
                               age_class == "Young of Year"~ "young_of_year",
                               TRUE ~ age_class))

#lat/lon
ia_fishobs %>% 
  summarise(min_lat = min(centroid_lat, na.rm = T),
            min_lon = min(centroid_lon, na.rm = T),
            min_unspec_lat = min(lat_unspec, na.rm = T),
            min_unspec_lon = min(lon_unspec, na.rm = T))

#batch weight and units
ia_fishobs %>% 
  filter(!is.na(batch_weight)) %>% 
  group_by(total_effort_ident, sub_effort_ident, species_1, batch_weight) %>% 
  count() 
#looks like there are many batch weights within a sub effort idnet
#it is probably net specific - how should we incorporate that into our structure? 

check <- ia_fishobs %>%
  group_by(sub_effort_ident) %>% 
  filter(any(!is.na(batch_weight))) %>% 
  ungroup() %>% 
  distinct(total_effort_ident, sub_effort_ident, species_1, batch_count_ident ,batch_weight)
rm(check)

ia_fishobs %>% 
  filter(total_effort_ident == "6091" & sub_effort_ident == "10F4963C-1E49-4277-B20B-CE9156BE1B52") %>% 
  group_by(total_effort_ident, sub_effort_ident, species_1, batch_count_ident ,batch_weight) %>% 
  count() %>% 
  arrange(batch_count_ident)
#is the count generated as the same count in the original batch count?
ia_fishobs_30Nov2023 %>% 
  filter(sub_effort_ident == "10F4963C-1E49-4277-B20B-CE9156BE1B52" & !is.na(batch_count_ident)) %>% 
  select(sub_effort_ident, species.1, batch_count_ident, batch_weight, individual_count) %>% 
  arrange(batch_count_ident)

#lets just retain the batch_count_ident in a notes columns so that a user would be able to make sense of the batch counts
ia_fishobs <- ia_fishobs %>% 
  mutate(notes_1 = paste0("Batch_Count_Ident: ", batch_count_ident, "; ", notes_1))

ia_fishobs %>% 
  filter(total_effort_ident == "6091") %>% 
  group_by(notes_1) %>% 
  count()
#we can see that batch count ident gets added to notes 1, even if it were na
#this allows it to be extracted by a user if batch weight were important 


#creating auto flags for effort
flag_xwalk <- ia_fishobs %>% 
...
flag_xwalk %>% 
  filter(!(str_detect(flag, "Agency flagged"))) %>% 
  filter(!is.na(flag))
...
#fish level flags for length and weight 
fish_length_weight_flag_xwalk <- ia_fishobs %>% 
...
fish_length_weight_flag_xwalk %>% 
  group_by(flag) %>% 
  count() %>% 
  print(n = nrow(.))

fish_length_weight_flag_xwalk %>% 
  filter(str_detect(flag, "length or weight reported a")) %>% 
  summarise(no_length = sum(length_1 == "0"),
            no_weight = sum(weight_1 == "0"))
#most of the 0s are coming from the weight column 

check <- fish_length_weight_flag_xwalk %>% 
  filter(str_detect(flag, "unlikely")) %>% 
  select(lake_name_1, species_clean, length_1, length_unit_1_clean, weight_1, weight_unit_1_clean, mean_length, mean_weight, flag)
rm(check)

ia_fishobs <- ia_fishobs %>% 
  select(-flag) %>% 
  left_join(fish_length_weight_flag_xwalk, relationship = "many-to-one")
rm(fish_length_weight_flag_xwalk)

#making na total effort idents when the effort is na
ia_fishobs <- ia_fishobs %>% 
  mutate(total_effort_ident = case_when(is.na(total_effort_1) ~ NA,
                                        TRUE ~ total_effort_ident))
ia_fishobs %>% 
  group_by(is.na(total_effort_ident), is.na(total_effort_1)) %>% 
  count()

ia_fishobs %>% 
  mutate(na_effort = case_when(is.na(total_effort_1) ~ T,
                               !is.na(total_effort_1) ~ F)) %>% 
  summarise(prop = mean(na_effort))

#######flag for multiple gears in a tei###############
#total effort ident is create at the consolidated gear level, there are a select number of surveys where the gear reported by agency compete with eachother. These were usually consistent with a total effort ident, but there are a few where the consolidated gear change at the more specific agency level within the survey_id, survey_start_date, lake_name level. We will just flag these so they will be removed. (above, consolidated gear is the more specific gear )
ia_fishobs %>% 
  group_by(total_effort_ident, lake_name_1, survey_start_date, survey_id, consolidated_gear) %>% 
  summarise(n_detailed = n_distinct(sampling_method_agency)) %>% 
  filter(n_detailed > 1)

multi_gears_flag <- ia_fishobs %>% 
  group_by(total_effort_ident, lake_name_1, survey_start_date, survey_id, consolidated_gear) %>% 
  mutate(flag = case_when(n_distinct(sampling_method_agency) > 1 ~ paste0("contrasting gears reported by agency within TEI", "; ", coalesce(flag, "")),
                          TRUE ~ coalesce(flag, "")),
         flag = na_if(flag, "")) %>% 
  ungroup() %>% 
  distinct() %>% 
...
multi_gears_flag %>% 
  filter(total_effort_ident %in% c("1137", "2657")) %>% 
  group_by(total_effort_ident, flag) %>% 
  count()

ia_fishobs <- ia_fishobs %>% 
  select(-flag) %>% 
  left_join(multi_gears_flag, relationship = "many-to-one")
rm(multi_gears_flag)
...
ia_fishobs %>%
  ...
  summarise(
    n_samples = n(),
    n_flagged = sum(str_detect(flag, "Agency flagged sample issue"), na.rm = T),
...
ia_fishobs %>% 
  filter(str_detect(flag, "contrasting")) %>% 
  group_by(total_effort_ident) %>% 
  count()
#now we can see flags for the appropriate total effort idents

#######creating cpue invalid column#################
ia_fishobs %>% 
  group_by(flag) %>% 
  count() %>% 
  print(n = nrow(.))
...
ia_fishobs <- ia_fishobs %>%
  mutate(cpue_invalid = case_when(str_detect(flag, "high effort") ~ "y",
                                  str_detect(flag, "Agency flagged") ~ "y",
                                  str_detect(flag, "contrasting gears") ~ "y",
                                  TRUE ~ "n")) 

#cleaning up aging structure and waterbody column
ia_fishobs %>% 
  distinct(aging_structure_1)

ia_fishobs <- ia_fishobs %>% 
  mutate(aging_structure_1 = case_when(aging_structure_1 == "" ~ NA,
         aging_structure_1 == "OTOLITH (asteriscus)" ~ "otolith_asteriscus",
         aging_structure_1 == "OTOLITH (sagittal)" ~ "otolith_sagittal",
         aging_structure_1 == "DORSAL SPINE" ~ "spine_dorsal",
         aging_structure_1 == "PECTORAL SPINE (articulating)" ~ "spine_pectoral_articulating",
         aging_structure_1 == "FISH" ~ "fish",
         aging_structure_1 == "PECTORAL SPINE (basal)" ~ "spine_pectoral_basal",
         aging_structure_1 == "DORSAL SPINE|GENETIC SAMPLE" ~ "spine_dorsal_genetic_sample",
         aging_structure_1 == "GENETIC SAMPLE" ~ "genetic_sample",
         aging_structure_1 == "GENETIC SAMPLE|OTOLITH (sagittal)" ~ "otolith_sagittal_genetic_sample",
         aging_structure_1 == "DORSAL SPINE|GENETIC SAMPLE|OTOLITH (sagittal)" ~ "spine_dorsal_otolith_sagittal_genetic_sample",
         aging_structure_1 == "PELVIC FIN" ~ "fin_pelvic",
         aging_structure_1 == "DORSAL SPINE|PELVIC FIN" ~ "spine_dorsal_fin_pelvic",
         aging_structure_1 == "SCALE" ~ "scale",
         aging_structure_1 == "FISH|OTOLITH (sagittal)" ~ "otolith_sagittal_fish",
         aging_structure_1 == "CLEITHRA" ~ "cleithra",
         aging_structure_1 == "OTOLITH (lapillus)" ~ "otolith_lapillus",
         TRUE ~ aging_structure_1)) 

#cleaning up waterbody_type 
ia_fishobs <- ia_fishobs %>% 
  mutate(lake_type = case_when(lake_type == "" ~ NA,
                               TRUE ~ lake_type)) 

#####cleaning up some total effort and sub effort items
ia_fishobs %>% 
  summarise(problem = sum(!is.na(sub_effort_ident) & is.na(sub_effort_1)),
            problem_2 = sum(is.na(total_effort_ident) & !is.na(total_effort_nothing_caught)))

ia_fishobs <- ia_fishobs %>% 
  mutate(sub_effort_ident = case_when(is.na(sub_effort_1) ~ NA,
                                      TRUE ~ sub_effort_ident),
         total_effort_nothing_caught = case_when(is.na(total_effort_ident) ~ NA,
                                                 TRUE ~ total_effort_nothing_caught)) 

#now we have a problem with sub
ia_fishobs %>% 
  summarise(problem = sum(is.na(sub_effort_ident) & !is.na(sub_effort_nothing_caught)))

ia_fishobs <- ia_fishobs %>% 
  mutate(sub_effort_nothing_caught = case_when(is.na(sub_effort_ident) ~ NA,
                                                 TRUE ~ sub_effort_nothing_caught)) 
...
ia_fishobs_trial <- ia_fishobs %>% 
  ...
  mutate(date_total_effort_ident = first(date_1),
         month = lubridate::month(date_total_effort_ident),
         year = lubridate::year(date_total_effort_ident)) %>% 
  ungroup() %>% 
  select(state, 
         county, 
         lake_name,
...
#exploring some funky na efforts
ia_fishobs_trial %>% 
  distinct(total_effort_ident, .keep_all = T) %>% 
  filter(is.na(total_effort_1) & is.na(flag)) %>% 
  group_by(total_effort_ident) %>% 
...
rm(ia_fishobs_30Nov2023, b, cde, ia_age_length_21Aug2021, ia_BLG_age_length_21Aug2021,ia_CCF_age_length_21Aug2021, ia_diet_21Aug2021, ia_lake_characteristics_21Aug2021, names, unusedbits, samplestationlocationmap_drawrectangletofilterlocations, filei, files_list, first_word, i, maxn, n, check)
```
...
halk %>%
  filter(state == "Iowa") %>% 
  summarise(n = sum(alk_length == 0, na.rm =T)) 
...
ia_fishobs_trial <- ia_fishobs_trial %>% 
