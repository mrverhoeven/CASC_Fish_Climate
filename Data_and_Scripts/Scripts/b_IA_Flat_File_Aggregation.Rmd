...
#no effort surveys without flags
ia_fishobs %>% 
...
check <- ia_fishobs %>% 
  ...
  distinct(total_effort_ident, lake_name_1, survey_start_date, survey_id, total_effort_1, total_effort_2, sub_effort_1, sub_effort_2, sampling_method_simple, sampling_method_1, sampling_method_2)

no_effort_flags <- ia_fishobs %>% 
  filter(is.na(total_effort_1) & is.na(flag)) %>%
  distinct(total_effort_ident, lake_name_1, survey_start_date, survey_id, total_effort_1, total_effort_2, sub_effort_1, sub_effort_2, sampling_method_simple, sampling_method_1, sampling_method_2, flag) %>% 
  group_by(total_effort_ident) %>% 
  mutate(flag = case_when(n_distinct(sampling_method_1) > 1 ~ "multipule sampling gears listed by the agency",
                          is.na(total_effort_1)
                          TRUE ~ flag))
###########stopped right above here creating flags for why there are no total efforts######
