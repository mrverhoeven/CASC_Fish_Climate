...
## Data Identifiers--these are ARCHIVED DATA lines
...
ia_fishobs[ , gear_data_notes_1 := ifelse(rowSums(!is.na(.SD)) > 0, apply(.SD, 1, function(x) paste(na.omit(x), collapse = "_")), NA), .SDcols = cols]
...
# a <- ia_fishobs
ia_fishobs <- clean_names(ia_fishobs)

#then print names, and paste into excel
# fwrite(as.data.table(names(ia_fishobs)), file = "Data_and_scripts/Data/output/ia_names.csv")
ia_renamer <- fread( file = "Data_and_scripts/Data/input/ia_renamer.csv")
...
rm(ia_fishobs_30Nov2023, b, cde, ia_age_length_21Aug2021, ia_BLG_age_length_21Aug2021,ia_CCF_age_length_21Aug2021, ia_diet_21Aug2021, ia_lake_characteristics_21Aug2021, names, unusedbits, samplestationlocationmap_drawrectangletofilterlocations, ia_renamer, addcols, dropcols, filei, files_list, first_word, i, maxn, n)
...
ia_fishobs[ , length(unique(paste(lake_name))) , .("Has_state_lake_ID" = !is.na(lake_id), "Has_state_lake_ID" = !is.na(nhdhr_id)) ]
...
ia_catch_eff <- as_arrow_table(ia_fishobs)
