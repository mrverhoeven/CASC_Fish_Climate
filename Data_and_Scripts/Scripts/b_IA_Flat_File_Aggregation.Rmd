...
#right from the start there are 7183 fish that have an aging strcuture that do not have an age
ia_fishobs_30Nov2023 %>% 
  mutate(aging_structure.1 = case_when(aging_structure.1 == "" ~ NA,
                                       TRUE ~ aging_structure.1)) %>% 
  filter(is.na(age) & !is.na(aging_structure.1))
...
ia_fishobs_trial <- ia_fishobs %>% 
  ...
  group_by(total_effort_ident) %>% 
  mutate(date_total_effort_ident = min(date_1, na.rm = T),
...
#this matches the number of fish removed due to length
