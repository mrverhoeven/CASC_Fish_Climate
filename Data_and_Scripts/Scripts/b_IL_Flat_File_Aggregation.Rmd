...
#Data Review
#this code was used as exploration in several iterations of the ag process - is not important for the aggregation and does not fully run
```{r}
...
il_data <- il_data.prep %>% 
  ...
  mutate(mean_effort = mean(total_effort_1, na.rm = TRUE),
         ...
         flag = case_when(total_effort_1 == 0 ~ paste("effort reported as 0, typical sets are for 1440 min but cannot be certain", flag),
                          total_effort_1 < 0 ~ paste("invaild effort (negative)", flag),
                     z_score > 10 ~ paste("high effort", flag),
                     TRUE ~ flag)) %>% 
  ungroup() %>% 
