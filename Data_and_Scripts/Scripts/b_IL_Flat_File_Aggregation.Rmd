...
il_data <- il_data.prep %>% 
  ...
  mutate(date_sub_effort_ident = min(date_sample, na.rm = T)) %>% 
  ungroup() %>% 
  group_by(total_effort_ident) %>% 
...
il_data %>% 
  filter(total_effort_ident == "1112.2") %>% 
  distinct(date_survey, date_total_effort_ident, date_sub_effort_ident, date_sample)
``` 
