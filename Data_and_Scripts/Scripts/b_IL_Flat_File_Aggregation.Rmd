...
survey.effort <- il_effort_17Jan22 %>% 
  ...
  group_by(lake_id, year, date.1, sampling_method, gear_data_notes.1, total_effort_2, total_effort_1, original_file_name.1) %>% 
  ...
  group_by(lake_id, year, sampling_method, gear_data_notes.1, original_file_name.1) %>% 
...
survey.effort %>% 
  group_by(lake_id, year, sampling_method, gear_data_notes.1, survey_effort) %>% 
  count() %>% 
  group_by(lake_id, year, sampling_method, gear_data_notes.1) %>% 
  count() %>% 
  filter(n >1)
...
il_data <- left_join(survey.effort, il_catch_age_effort_17Jan22, by = c("lake_id",
                                                                        "year", 
                                                                        "date.1", 
                                                                        "sampling_method", 
                                                                        "gear_data_notes.1",
...
il_data %>% 
  ...
  group_by(caught_nothing, lake_name_1, lake_id, sub_effort_1_id) %>% 
  count() %>% 
  print(n = nrow(.))
...
cpue <- il_data %>% 
  ...
  distinct(sampling_method_1, total_effort_1_id, total_effort_1, n, species_1, cpue)
