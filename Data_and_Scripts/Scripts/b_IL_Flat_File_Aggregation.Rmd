...
effort.check <- il_data.catch %>% 
  ...
  select(survey_id, lake_id, lake_name.1, year, date.1, gear_data_notes.1, no_target, target_species, target_species_1, target_species_2, target_species_3, survey_effort_id, sub_effort_ident, total_effort_2, total_effort_1, survey_effort, subsampling_survey, flag)
...
il_data <- bind_rows(il_data_merged_age %>% 
  ...
  #creating better effort identifers
...
il_data <- il_data %>% 
  ...
  rename(sampling_method.big = sampling_method.1) %>% 
  rename(sampling_method = sampling_method_agg) %>% 
  rename(sampling_method.1 = gear_data_notes.1) %>% 
  mutate(sampling_method.1 = gsub("Gear Used:", "", sampling_method.1)) %>% 
  mutate(sampling_method = gsub("Gear Used:", "", sampling_method)) %>% 
...
example <- il_data %>% 
  filter(lake_name == "Baldwin Lake" & year == "2012") %>% 
  distinct(lake_name, date, total_effort_ident, sub_effort_ident, no_target, target_species, total_effort_1)
...
example.cpe <- il_data %>% 
  ...
  distinct(lake_name, date, total_effort_ident, sub_effort_ident, no_target, target_species, total_effort_1, .keep_all = T) %>% 
  select(lake_name, date, total_effort_ident, sub_effort_ident, no_target, target_species, total_effort_1, count.blg, count.cc, count.lmb, cpe.blg, cpe.cc, cpe.lmb)
...
il_data %>% 
  distinct(total_effort_ident, survey_type_1) %>% 
  group_by(survey_type_1) %>% 
  count()

il_data %>% 
  filter(sub_survey_nothing_caught == T) %>% 
  distinct(total_effort_ident, sub_effort_ident, survey_type_1) %>% 
  group_by(survey_type_1) %>% 
  count()

il_data %>% 
  group_by(sampling_method, sampling_method_1) %>% 
  count()
...
il_data %>% 
  filter(total_effort_ident == "497.2") %>% 
  distinct(lake_name, total_effort_ident, sub_effort_ident, total_effort_1, sub_effort_1) %>% 
  collect() %>% 
  print(n = nrow(.))
``` 
