...
-naming schema: -made net/run (agency provided) into site_id 
                -moved sub sampling desgination to gear data notes 
...
il_data <- il_data.prep %>% 
  ...
  mutate(nhdr_id = as.character(NA),
         year = year(date.1),
         month = month(date.1),
         ...
         batch_weight_units = as.character(NA),
...
example <- il_data %>% 
  filter(lake_name == "Baldwin Lake" & year == "2012") %>% 
  distinct(lake_name, date, total_effort_ident, sub_effort_ident, target_species, total_effort_1)
...
example.cpe <- il_data %>% 
  ...
  distinct(lake_name, date, total_effort_ident, sub_effort_ident, target_species, total_effort_1, .keep_all = T) %>% 
  select(lake_name, date, total_effort_ident, sub_effort_ident, target_species, total_effort_1, count.blg, count.cc, count.lmb, cpe.blg, cpe.cc, cpe.lmb)
