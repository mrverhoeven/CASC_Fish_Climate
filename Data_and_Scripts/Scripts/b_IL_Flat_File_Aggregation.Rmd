...
files_list <- list.files(path = "G:/Shared drives/Hansen Lab/RESEARCH PROJECTS/Fish Survey Data/IL_Data/il_raw_disaggregated_data", pattern = ".+\\.csv") #grabs only.csv files
files_list
...
for(i in 1:n) {
  ...
  assign(filei ,
          fread(paste0("G:/Shared drives/Hansen Lab/RESEARCH PROJECTS/Fish Survey Data/IL_Data/il_raw_disaggregated_data/",
                                          files_list[i])))
...
il_catch_age_effort_30Nov23_uncount <- il_catch_age_effort_30Nov23 %>% 
  ...
  mutate(length_bin = case_when((total_count.1 > 1 & (length.1 %% 10 == 0))  ~ length.1,
                                TRUE ~ NA),
         length_bin = case_when(!is.na(length_bin) ~ paste0(length_bin, "-", length_bin+9.99),
                                TRUE ~ NA),
         length_bin_unit = case_when(!is.na(length_bin) ~ "millimeters",
                                     TRUE ~ NA),
...
il_catch_age_effort_30Nov23_uncount %>% 
  distinct(length.1, length_bin, length_bin_unit) %>%  
  print(n = nrow(.))
...
rm(effort.check, excluded, no_fish, no_matches, surveys.excluded)
``` 
...
il_data.prep %>% 
   mutate(weight_unit.1 = case_when(weight_unit.1 == "col_name_Wt(g)" ~ "grams",
                                   TRUE ~ NA)) %>% 
...
il_data.prep %>% 
  ...
  filter(!str_detect(length_bin, "0$")) %>% 
  distinct(length_bin, length_bin_unit, original_file_name.1_agedfish, original_file_name.1_effort, original_file_name.1_indvifish) %>%
  print(n = nrow(.))
...
###########creating column for cpue invalids###############
il_data.prep %>% 
  group_by(flag) %>% 
  count() 

il_data.prep %>% 
  group_by(is.na(survey_effort_id), flag) %>% 
  count()

il_data.prep <- il_data.prep %>% 
  mutate(cpue_invalid = case_when(str_detect(flag, "high effort") ~ "y",
                                  str_detect(flag, "effort reported as 0") ~ "y",
                                            TRUE ~ "n"))

#clean up aging structure column
il_data.prep <- il_data.prep %>% 
  mutate(aging_structure.1 = tolower(aging_structure.1),
         aging_structure.1 = case_when(aging_structure.1 == "" ~ NA,
                                       aging_structure.1 == "scales" ~ "scale",
...
il_data <- il_data.prep %>% 
  ...
  mutate(length_unit.1 = "millimeters") %>% 
...
il_data %>% 
  distinct(length_unit_1, weight_unit_1, length_bin_unit, batch_weight_unit)
``` 
