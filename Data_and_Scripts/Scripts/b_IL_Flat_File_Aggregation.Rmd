...
things do discuss:
 1. I did not use the lake info, effort, or age files
...
#only two surveys, 8 fish have na lengths - total count in these is 0?
...
multieffort.fish <- il_catch_age_effort_17Jan22 %>% 
  group_by(survey_id) %>% 
...
#merging age data to fish data
age.fish <- left_join(il_catch_age_effort_17Jan22, 
                      il_aged_fish_surveys_28Dec2022,
                      by = c("lake_id", 
                             "year", 
                             "date.1",
...
survey.effort <- il_catch_age_effort_17Jan22 %>% 
  group_by(survey_id, year, date.1, sampling_method, total_effort_2, total_effort_1) %>% 
  count() %>% 
  group_by(survey_id, year, sampling_method) %>% 
  mutate(survey_effort = sum(total_effort_1),
         survey_effort_id = cur_group_id())
...
check <- survey.effort %>% 
  filter(survey_id == c("Survey_880"))
#surveys 1324, 525, and 880 have two efforts for the survey id but they are in different years
#this does not hinder us with the join, but date/year will need to be included when counting


il_data <- left_join(il_catch_age_effort_17Jan22, survey.effort, by = c("survey_id", 
                                                                              "year", 
                                                                              "date.1", 
                                                                              "sampling_method", 
                                                                              "total_effort_2", 
...
il_data %>% 
  ...
  #sub-survey level effort is named total_effort_2 with sub-survey ids as survey_effort_2_id
  rename(total_effort_2_id = total_effort_2) %>% 
  rename(total_effort_1_id = survey_effort_id) %>% 
  rename(total_effort_2 = total_effort_1) %>% 
