...
il_effort_30Nov23_target <- il_effort_30Nov23 %>% 
  group_by(
    survey_id,
    lake_id,
    year,
...
survey.effort <- il_effort_30Nov23_target %>% 
...
effort.check <- il_data.catch %>% 
  ...
  select(survey_id, lake_id, lake_name.1, year, date.1, gear_data_notes.1, survey_effort_id, total_effort_2, total_effort_1, survey_effort)
...
il_aged_fish_surveys_12Dec2023 %>% 
  ...
  count()
#no subsample times so we don't have to worry about that
...
#no surveys have sub-sampling associated with them - thus I am just making a subsample time 1 columns where all values will be 0
...
il_data_merged_age <- il_data_merged_age %>% 
  ...
  mutate(survey_type.1 = garbage_bin_notes.2) %>% #making survey time consistent with the first catch file
  mutate(flag = case_when(total_effort_1 == 0 ~ "effort not reported, typical sets are for 1440 min but cannot be certain",
...
il_data <- bind_rows(il_data_merged_age %>% 
  ...
  mutate(survey_effort_id = paste0(survey_effort_id, "_", source))
...
il_data %>% 
  ...
  count()
#looks good - all surveys from the age file have an x_1 while all surveys from the catch file have an an x_2
...
il_data %>% 
  ...
  count() %>% 
  print(n = nrow(.))
#here we can see many files from the aged fish data share a survey id - checking with vic about that
...
il_data <- il_data %>% 
  ...
  mutate(length_unit.1 = "unspecified") %>% 
  ...
  #cleaning up sub sample species and time
  mutate(subsample_time_1 = gsub("Subsample Time:", "", subsample_time_1),
  ...
  mutate(target_species_1 = case_when(target_species_1 %in% c("", "0") ~ NA,
...
il_data %>% 
  ...
  select(total_effort_ident, total_survey_nothing_caught, sub_survey_nothing_caught) %>% 
  print(n = nrow(.))
...
il_data %>% 
  filter(total_effort_ident == "14_2") %>% 
  distinct(total_effort_ident, total_survey_nothing_caught, sub_survey_nothing_caught) %>% 
  select(total_effort_ident, total_survey_nothing_caught, sub_survey_nothing_caught)
...
```{r}
check <- il_data %>% 
  filter(total_effort_ident == "1_1") %>% 
  collect() %>% 
  distinct(date_1, total_effort_ident, total_effort_1, sub_effort_ident, sub_effort_1)
```
