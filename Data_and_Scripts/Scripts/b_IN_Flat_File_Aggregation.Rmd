...
in_glaciallakes_effort_6Oct2021_no_ID_match <- in_glaciallakes_effort_6Oct2021 %>%
  left_join(in_lake_id_list_22Aug2022_fixed, by = c("lake_name.1", "county"))%>%
  filter(is.na(lake_id))%>%
  select(lake_name.1, county)%>%
  distinct()%>%
  left_join(in_lake_id_list_22Aug2022_fixed, by = c("lake_name.1"))


# leaving the lake ID issue behind for now... will either ask for fresh data with lake ids, or if data look okay, OR will manually work through it with janitor etc. 
```
