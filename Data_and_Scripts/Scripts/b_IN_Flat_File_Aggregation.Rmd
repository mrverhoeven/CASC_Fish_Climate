...
in_res_aged_effort_fish <- in_reservoir_age_effort_16Aug2022%>%
  ...
  mutate(survey_type.1 = if_else(survey_type.1 == "Reservoir S&T", "standard", "NA","targeted"))%>%
  select(survey_id,
         lake_name.1,
         lake_id,
         date.1,
         ...
         total_effort,
         effort_units)%>%
  full_join(in_reservoir_age_fish_16Aug2022, by = c("survey_id", "sampling_method"))

in_reservoir_age_fish_16Aug2022_merge  <-  in_reservoir_age_fish_16Aug2022 %>% # change this to in_res_aged_effort_fish
  ...
  arrange(survey_id, species.1, sampling_method.1, length.1, weight.1) %>%
  group_by(survey_id,species.1, sampling_method.1, length.1, weight.1) %>%
  mutate(age_id = row_number()) %>% #ask Denver about this
  ungroup() %>%
  mutate(length.1 = as.double(length.1),
         weight.1 = as.double(weight.1)) %>%
 mutate(sampling_method = case_when(sampling_method.1 == "Night EF" ~ "dc nighttime electrofishing",
                                     sampling_method.1 == "Gill net" ~ "gill net std exp",
                                     sampling_method.1 == "Trap net std" ~ "standard trap net",
                                     sampling_method.1 == "MITN" ~ "Michigan trap net",
                                     sampling_method.1 == "Large-mesh Gill Net" ~ "large-mesh GN",
                                    sampling_method.1 == "IN Standard Net" ~ "in standard net",
                                    .default = "unknown"))%>%
  mutate(sampling_method_2 = case_when(sampling_method == "dc nighttime electrofishing" ~ "Boat mounted DC nighttime electrofishing",
                                  sampling_method == "gill net std exp" ~ "250-ft x 6-ft, with 5 successive 50-ft panels including one panel of each: ¾ in., 1 in., 1-1/4 in., 1-1/2in., and 2 in square mesh ",
                                  sampling_method == "standard trap net" ~ "1-in mesh 45-ft lead; two rectangular 6-ft x 10-ft wire frames and four 2.5-ft diameter circular wire frames spaced 4-ft apart at the cod-end of the trap nets",
  ...
  left_join(in_reservoir_survey_dates, by = c("survey_id"))%>% #adds lake and date info for all aged fish
  select(survey_id,
         sampling_method,
         sampling_method_2,
         species.1,
...
in_age_check1 <- in_reservoir_data_age %>%
  select(survey_id, sampling_method, species_1, length_1, weight_1, age_id)
...
in_reservoir_tidy <- in_reservoir_data %>%
  mutate(original_file_names= paste(original_file_names, original_file_name.1_age, sep = ", ") )%>%
...
in_ALL_tidy <- in_ALL %>%
  ...
  mutate(survey_id = as.character(survey_id))%>%
  mutate(original_file_names = paste(original_file_names, original_file_name.1_age, sep = ", "))%>%
  mutate(gear_data_notes = paste(gear_data_notes_1, gear_data_notes.1, sep = ", "))%>%
  select(state,
         county,
         lake_name,
         lake_id,
