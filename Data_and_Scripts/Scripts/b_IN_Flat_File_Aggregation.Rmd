...
in_glaciallakes_effort_6Oct2021_2 <- in_glaciallakes_effort_6Oct2021 %>%
  ...
  mutate(sampling_method = if_else(sampling_method == "nighttime electrofishing", "boat_electrofishing_dc_night", sampling_method))
```
...
in_glaciallakes_standard_tidy <- in_glacial_lakes_standard_merge%>%
  ...
  mutate(flag = case_when(length.1 == 0 ~ "length of zero",
                          ...
                          .default = NA))%>%
  mutate(sampling_method_2 = case_when(sampling_method == "gill_net_experimental" ~ "250 x 6 ft; five 50-ft panel sections of 0.5-in, 1-in, 1.5-in, 2-in and 2.5-in mesh sizes",
                                       sampling_method =="boat_electrofishing_dc_night" ~ "Boat mounted DC nighttime electrofishing",
...
in_glaciallakes_targeted_inner_join_2 <- in_glaciallakes_targeted_fish_16Nov2023_2 %>%
  inner_join(in_glaciallakes_targeted_effort_16Nov2023_2, by = c("lake_id", "year"),
             suffix = c("_catch", "_effort")) %>% 
  select(lake_id, year, date.1_catch, date.1_effort) %>% 
