...
To do in reservoir:
1. targeted - why are there no fish for survey 512 standard traps despite showing significant effort ?
2. aged - can I just join to add age to the standard surveys? are they targeted surveys different?

#Resivor effort merge
```{r}
...
#aged fish
anti.age.effort <- anti_join(in_reservoir_age_effort_16Aug2022, 
                             in_reservoir_fish_community_effort_16Aug2022,
                             by = c("survey_id", "length.1", "weight.1", "species.1"))

in_reservoir_age_effort_16Aug2022 %>% 
  group_by(total_effort_4, total_effort_5) %>% 
  count()
...
in_reservoir_targeted_effort_1Sep2022 %>% 
  group_by(survey_type.1) %>% 
  count()

trial <- left_join(in_reservoir_fish_community_fishdata_16Aug2022, in_reservoir_age_fish_16Aug2022,
                   by = c("survey_id", "species.1", ))

#these fish appear be fish from the above surveys - can I just join on lake, sampling method, weight, date
#I think I would only be able to do this for the standard surveys - the effort for the targeted survey is not in the targeted effor file for effort 
