...
in_glaciallakes_standard_tidy <- in_glacial_lakes_standard_merge%>%
  ...
  mutate(flag = case_when(county.fish != county.effort ~ "multiple counties listed",
...
in_reservoir_fish_community_effort_16Aug2022_gear <- in_reservoir_fish_community_effort_16Aug2022 %>%
  ...
  mutate(effort_units = case_when(sampling_method == "boat_electrofishing_dc_night" ~ "hours",
                                  sampling_method == "gill_net_experimental_res" ~ "net_nights",
...
in_reservoir_fish_community_fishdata_16Aug2022_prep <- in_reservoir_fish_community_fishdata_16Aug2022 %>%
  ...
  #there are 13 blank species records from Lemon lake that show up - remove because they look erreonus 
  filter(!(lake_name.1 == "Lemon" & species.1 == ""))
```
...
in_targeted_species <- in.targeted %>%
  mutate(species.1 = str_to_lower(species.1))%>% #make species name lowercase
  mutate(species.1 = str_replace(species.1, " ", "_"))%>% #remove spaces from species name and put underscores
...
in_reservoir_age_fish_16Aug2022_merge  <- in_reservoir_age_fish_16Aug2022 %>% 
 ...
 mutate(sampling_method = case_when(sampling_method == "Night EF" ~ "boat_electrofishing_dc_night",
  ...
  #this removes two fish records that are in the age data file without ages
  filter(!(survey_id == "480" & is.na(age)) & !(survey_id == "489" & is.na(age)))
...
-a flag is added where there is a mismatch between efforts, fish, ages 
```{r}
...
in_age_check1 <- in_reservoir_data_age %>%
  select(survey_id, sampling_method, species_1, length_1, weight_1, age_id) #66284
...
in_age_check2 <- in_reservoir_age_fish_16Aug2022_merge %>%
  ...
  filter(!is.na(age)) #6005 fish with ages - 2 fish in the aged data do not have ages(filtered out above)
...
in_age_check3 <- left_join(in_reservoir_data_age,
                                  in_reservoir_age_fish_16Aug2022_merge,
                                    by = c("survey_id",
                                           ...
                                           "age_id")) %>%
  filter(!is.na(age)) 
#the full join added 3016 fish obs to the res data - meaning that many aged fish do not match preexisting records (2991/6007)
...
anti.age <- anti_join(in_reservoir_age_fish_16Aug2022_merge,
  ...
  select(survey_id,
         ...
         age_id,
         date_1)
#we have 6007 aged fish after the effort merge, 2991 matched with fish already in res surveys. Thus, we should get 3014 fish that do not match and are "extra fish". These will not be for abundance surveys but could be useful for ALKs

anti_join(in_reservoir_age_fish_16Aug2022_merge,
                      in_reservoir_data_age,
                                    by = c("survey_id",
                                           "sampling_method",
                                           "lake_id",
...
no_matches_surveys <- in_reservoir_age_fish_16Aug2022_merge %>% 
  ...
  select(survey_id,
         ...
         weight_1,
         age_id)
#surveys that have age data but no community data:  481, 482, 483, 484, 485, 486, 487, 488, 489, 490
#survey has community fish with effort but there are bluegill that don't match with weight/length: 379
...
#creating flag for surveys that don't match
in_reservoir_data %>% 
  filter(survey_id == 480 & lake_id == "{A25262D7-430C-4DC3-91D0-17E2F778EFDE}" & species_1 == "largemouth bass" & weight_1 == 53) %>% 
  select(survey_id,
         lake_id,
         species_1,
         weight_1,
...
in_reservoir_data  %>% 
  filter(survey_id == 489 & lake_id == "{C47CE045-4D52-4279-B982-E47E1409874F}" & species_1 == "walleye" & length_1 == 19.3) %>% 
  select(survey_id,
         lake_id,
         species_1,
         length_1,
         age,
         original_file_name.1_age)
#this shows that the two fish records without ages in the aged fish file are removed 


in_reservoir_data <- in_reservoir_data %>% 
  mutate(flag = case_when((!is.na(age) & is.na(total_effort_1_unaged) & is.na(total_effort_1age)) ~ "aged fish record missing effort data",
                          (!is.na(age) & is.na(total_effort_1_unaged) & !is.na(total_effort_1age)) ~ "effort retained from aged fish file, no match in community survey records",
                          TRUE ~ NA
                          )
         )
...
in_reservoir_data %>% 
  group_by(flag) %>% 
  count()
#3014 adds up to the number of aged fish that didn't match pre-existing fish
```
...
in_reservoir_tidy <- in_reservoir_data %>%
  ...
  mutate(date_1 = as.Date(date_1, "%m/%d/%y"), #converts character in aged to date 
         date = as.Date(date, "%m/%d/%y"), # converts Idate from normal to date
         date = case_when(is.na(date) ~ date_1,
                          TRUE ~ date), # there are some aged fish not in normal surveys where I need to fill 
         month = month(date),
         year = year(date)) %>% 
...
#this is where the show happens - finally getting the glacial lakes and res data row binded 
...
in_data_prep <- in_data_prep %>% 
  ...
  mutate(latitude_lake_centroid = lat_unspec,
         ...
         latitude_lake_centroid = case_when(is.na(latitude_lake_centroid) ~ centroid_lat,
...
coord_check <- in_data_prep %>% 
  distinct(lake_id, .keep_all = T) %>% 
  select(lake_id, lake_name, nhdhr_id, latitude_lake_centroid, centroid_lat, longitude_lake_centroid, centroid_lon) %>% 
  mutate(diff_lat = abs(latitude_lake_centroid - centroid_lat),
         diff_lon = abs(longitude_lake_centroid - centroid_lon))
#looks like there is a typo in the state agency file for Hindman centroid lat. Let me just grab the msu centroid for this lake then

in_data_prep <- in_data_prep %>% 
  #for only Hindman lake take the centroid - all other lakes return what is already given
  mutate(latitude_lake_centroid = case_when(nhdhr_id == "nhdhr_155642913" ~ centroid_lat,
                                            TRUE ~ latitude_lake_centroid),
         longitude_lake_centroid = case_when(nhdhr_id == "nhdhr_155642913" ~ centroid_lon,
                                             TRUE ~ longitude_lake_centroid))

in_data_prep %>% 
  filter(nhdhr_id == "nhdhr_155642913") %>% 
  distinct(latitude_lake_centroid, longitude_lake_centroid,) %>% 
  mutate(as.character(latitude_lake_centroid), as.character(longitude_lake_centroid))
...
gear_xwalk %>% 
  group_by(sampling_method_agency, sampling_method_2) %>% 
  count()

in_data_prep %>% 
  rename(sampling_method_agency = sampling_method) %>% 
  left_join(gear_xwalk, by = "sampling_method_agency", relationship = "many-to-one") %>% 
  group_by( sampling_method_simple, sampling_method_1, sampling_method_2) %>% 
  count()
...
in_data_prep %>%
   distinct(total_effort_ident, total_effort_1, .keep_all = T) %>% 
   group_by(total_effort_ident, lake_name, lake_id, sampling_method_simple, year) %>% 
   count() %>% 
   filter(n > 1)
#there are  6 surveys that have the same sampling method survey within a lake in a year - need to fix the ident
  ## this really manifests by seeing there is more than one total effort within a total effort ident based on the date
  #for the sake of ease, I am going to assign different total effort idents for each of these
  #an argument could be made that Hardy dc night efishing in 2010 should be lumped together and have sub-effort idents but this is such a small set of data that it's not worth the fuss. A user could also do this on their own if they wanted
in_data_prep <- in_data_prep %>% 
  #need to make total effort ident string first
  mutate(total_effort_ident = as.character(total_effort_ident)) %>% 
  mutate(total_effort_ident = case_when(lake_name == "Patoka" & year == "2018" & sampling_method_simple == "gill_net" ~ paste0(total_effort_ident, "-", total_effort_1),
                                        lake_name == "Patoka" & year == "2013" & sampling_method_simple == "boat_electrofishing" ~ paste0(total_effort_ident, "-", total_effort_1),
                                        lake_name == "Patoka" & year == "2016" & sampling_method_simple == "boat_electrofishing" ~ paste0(total_effort_ident, "-", total_effort_1),
                                        lake_name == "Hardy" & year == "2010" & sampling_method_simple == "boat_electrofishing" ~ paste0(total_effort_ident, "-", total_effort_1),
                                        lake_name == "Hardy" & year == "2018" & sampling_method_simple == "frame_net" ~ paste0(total_effort_ident, "-", total_effort_1),
                                        lake_name == "Brookville" & year == "2018" & sampling_method_simple == "frame_net" ~ paste0(total_effort_ident, "-", total_effort_1),
...
in_data_prep %>% 
  filter(str_detect(total_effort_ident, "-")) %>% 
  distinct(total_effort_ident) %>% 
  arrange(total_effort_ident)


#auto generating flags - first for effort and second for individual fish
effort_flags_xwalk <- in_data_prep %>% 
  distinct(total_effort_ident, flag, age, species_1, length_1, weight_1, .keep_all = T) %>% 
...
in_data_prep <- in_data_prep %>% 
  select(-flag) %>% 
  left_join(effort_flags_xwalk, relationship = "many-to-one")
...
#individual fish length and weight flag
length_weight_flag <- in_data_prep %>% 
...
in_data_prep <- in_data_prep %>% 
  select(-flag) %>% 
  left_join(length_weight_flag, relationship = "many-to-one")

in_data_prep %>% 
  group_by(flag) %>% 
  count()

#length/weight columns
in_data_prep %>% 
  group_by(length_unit_1,
           weight_unit_1) %>% 
  count()
...
in_data_prep <- in_data_prep %>% 
  mutate(length_unit_1 = case_when(length_unit_1 %in% c("col_name_Length (in)", "inches", "knownunits_inches") ~ "inches",
                                   TRUE ~ length_unit_1),
         weight_unit_1 = case_when(weight_unit_1 == "col_name_weight (lb)" ~ "pounds",
                                   TRUE ~ weight_unit_1),
         length_unit_1 = case_when(!is.na(length_1) ~ length_unit_1,
                                   TRUE ~ NA),
...
##############aging str#####################################
in_data_prep <- in_data_prep %>% 
  mutate(aging_structure_1 = case_when(aging_structure_1 == "Dorsal spine" ~ "dorsal_spine",
                                       aging_structure_1 %in% c("Otolith", "Otoliths") ~ "otolith",
...
in_data_prep %>% 
  ...
  group_by(flag) %>% 
  count()
#Aged fish record that were not matched to community surveys AND did not have effort from the age file have NA date, month, and year

in_data_prep %>% 
  filter(is.na(species_1)) %>% 
  glimpse()

#############survey type standardization###########
#make sure all surveys have a survey type
#also make sure that extra aged fish get a different survey type 
#remove total effort idents from extra fish that should not be used for cpe generation
in_data_prep <- in_data_prep %>% 
  mutate(survey_type = case_when(str_detect(flag, "aged") ~ "aged_fish",
                                 TRUE ~ survey_type)) %>% 
  mutate(total_effort_ident = case_when(str_detect(flag, "aged") ~ NA,
                                 TRUE ~ total_effort_ident))
...
in_data_prep <- in_data_prep %>% 
  mutate(cpue_invalid = case_when(str_detect(flag, "effort") ~ "y",
                                  str_detect(flag, "no sampling method") ~ "y",
                                  TRUE ~ "n")) 

#cleaning up aging structure
in_data_prep <- in_data_prep %>% 
  mutate(aging_structure_1 = case_when(aging_structure_1 == "dorsal_spine" ~ "spine_dorsal",
...
###total effort/nothing caught NA cleaning
in_data_prep %>% 
  summarise(problem = sum(!is.na(total_effort_ident) & is.na(total_effort_1)),
            n_tei = n_distinct(total_effort_ident))
#we have some total effort idents that do not have effort
...
in_data_prep <- in_data_prep %>% 
  #creates nothing caught column
  mutate(nothing_caught = if_else(is.na(species_1), T, F))%>%
  mutate(nothing_caught = case_when(is.na(total_effort_ident) ~ NA,
                                    TRUE ~ nothing_caught))

##########################final column prep##################################
in_data <- in_data_prep %>%
  #fixing species if it hasn't been already
  mutate(species_1 = case_when(species_1 == "longnose_gar gar" ~ "longnose_gar",
                               TRUE ~ species_1),
         species_1 = str_to_lower(species_1))%>%
  mutate(species_1 = str_replace(species_1, " ", "_"))%>%
  mutate(date = as.Date(date)) %>% 
  #this is probs already done, but making sure all records have a year and month
  mutate(month = case_when(!is.na(date) ~ month(date),
                           TRUE ~ month),
         year = case_when(!is.na(date) ~ year(date),
                          TRUE ~ year)) %>% 
  #cleaning up lake size units
  mutate(lakesize_units = case_when(c_lakesize_units == "col_name_SurfaceAcres" ~ "acres",
                                    TRUE ~ c_lakesize_units)) %>% 
  #making sure we have created all columns for the hive
  mutate(date_survey = date,
...
in_data %>% 
  ...
  group_by(lake_name, sampling_method, survey_type, date_survey, date_total_effort_ident, month, year, species_1, total_effort_1) %>% 
  count()
...
in_data %>% 
  group_by(sampling_method, total_effort_1_units, is.na(total_effort_1)) %>% 
  count()
...
in_data %>% 
  ...
  group_by(waterbody_type, survey_type, total_effort_ident, lake_name, sampling_method, date_total_effort_ident, flag, survey_id) %>% 
  count()


in_data %>% 
  filter(str_detect(flag, "high effort")) %>% 
...
in_data %>% 
  filter(!is.na(total_effort_1) & is.na(total_effort_1_units)) %>% 
  group_by(total_effort_ident, sampling_method, waterbody_type, survey_type) %>% 
  count() %>% 
  print(n = nrow(.))
...
in_glaciallakes_fish_6Oct2021 %>% 
  filter(lake_name.1 == "Flint" & year == "2013") %>% 
  glimpse()
#only two fish caught in Flint lake via electrofishing - just a gut check on our total effort nothing caught 
...
in_data %>% 
  group_by(is.na(date_survey), is.na(date_total_effort_ident), is.na(month), is.na(year), is.na(flag)) %>% 
  count()
...
1. Based on the current total effort ident groupings, there are six lake-sampling methods that happen more than once in a year. Each sampling event has a different total effort
    -For ease, I made each sampling event its own total effort ident (pasted total effort ident to the total effort). We could conciser making sub effort idents that populate total effort idents 
...
3. flags
  - flags are created at the effort and fish level
