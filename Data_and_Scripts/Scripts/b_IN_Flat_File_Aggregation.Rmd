...
in_reservoir_fish_community_effort_16Aug2022_gear <- in_reservoir_fish_community_effort_16Aug2022 %>%
  ...
  mutate(sampling_method = case_when(sampling_method == "total_effort_1" ~ "boat_electrofishing_dc_night",
                                     sampling_method == "total_effort_2" ~ "gill_net_experimental",
...
in_reservoir_fish_community_fishdata_16Aug2022_prep <- in_reservoir_fish_community_fishdata_16Aug2022 %>%
  mutate(sampling_method = case_when(sampling_method.1 == "Gill net STB exp" ~ "gill_net_experimental",
                                     sampling_method.1 == "Gill net std exp" ~ "gill_net_experimental",
...
in.targeted <- inner_join(in_reservoir_targeted_effort_1Sep2022_gear, 
  ...
  mutate(sampling_method = case_when(sampling_method == "Night EF" ~ "boat_electrofishing_dc_night",
                                     sampling_method == "Gill net std exp" ~ "gill_net_experimental",
...
in_reservoir_age_fish_16Aug2022_merge  <- in_reservoir_age_fish_16Aug2022 %>% 
 ...
 mutate(sampling_method = case_when(sampling_method == "Night EF" ~ "boat_electrofishing_dc_night",
                                     sampling_method == "Gill net" ~ "gill_net_experimental",
                                     sampling_method == "Trap net std" ~ "trap_net",
                                     sampling_method == "MITN" ~ "trap_net_michigan",
...
in_data <- in_data_prep %>%
  ...
  mutate(month = case_when(!is.na(date) ~ month(date),
         ...
         year = case_when(!is.na(date) ~ year(date),
                          TRUE ~ year)) %>% 
  #making simple gear type 
  mutate(sampling_method_simple = case_when(sampling_method == "boat_electrofishing_dc_night" ~ "boat_electrofishing",
                                            sampling_method == "gill_net_experimental" ~ "gill_net",
                                            sampling_method == "gill_net_large_mesh" ~ "gill_net",
                                            sampling_method == "standard_net_in" ~ "frame_net",
                                            sampling_method == "trap_net" ~ "frame_net",
                                            sampling_method == "trap_net_michigan" ~ "frame_net",
                                            sampling_method == "unknown_sampling_method" ~ "unknown",
                                            TRUE ~ NA),
         sampling_method_2 = case_when(sampling_method == "boat_electrofishing_dc_night" ~ "Boat mounted DC nighttime electrofishing",
                                            sampling_method == "gill_net_experimental" ~ "250 x 6 ft; five 50-ft panel sections of 0.5-in, 1-in, 1.5-in, 2-in and 2.5-in mesh sizes",
                                            sampling_method == "gill_net_large_mesh" ~ "250-ft x 6-ft, with 2.25-in bar mesh",
                                            sampling_method == "standard_net_in" ~ "unknown specs",
                                            sampling_method == "trap_net" ~ "1-in mesh 45-ft lead; two rectangular 6-ft x 10-ft wire frames and four 2.5-ft diameter circular wire frames spaced 4-ft apart at the cod-end of the trap nets",
                                            sampling_method == "trap_net_michigan" ~ "Michigan trap net",
                                            sampling_method == "unknown_sampling_method" ~ "unknown specs",
                                            TRUE ~ NA)) %>% 
...
2. There are some records with NA effort
    -five glacial lake records that have unknown sampling methods that cannont be linked to an effort
    -some aged fish records that do not match with effort records 
       ...
       -this leads to most survey level info disappering - since there is a survey_id perhaps I can pull from that?
       
3. effort flag
    -it is currenlty based on fish obs level effort (surveys with more fish have more weight) need to fix but was struggling to figure out
