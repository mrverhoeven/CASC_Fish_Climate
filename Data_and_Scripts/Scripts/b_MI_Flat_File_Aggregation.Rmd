...
#more aged fish than desired?
mi_catch_eff_merge %>% 
  group_by(species.1, is.na(age)) %>% 
  count() %>% 
  print(n = nrow(.))
#we can see that there are more species within the aged fish file than in the catch file - lets remove them

mi_catch_eff_merge <- mi_catch_eff_merge %>% 
  filter(!(str_detect(species.1, "taxon")) | is.na(species.1)) 

mi_catch_eff_merge %>% 
  group_by(species.1, is.na(age)) %>% 
  count() %>% 
  print(n = nrow(.))
#now we can see that there are only the 8 sport fish species, but they include aged fish as well

#since we are not able to retain effort information with aged fish, lets remove the total effort ident from them
mi_catch_eff_merge %>% 
  group_by(is.na(age), is.na(total_effort_1_effort), is.na(effort_ident)) %>% 
  count()
#here we can see all records get assigned a total effort ident, even if total effort is na - lets remove the total effort ident when effort is NA

mi_catch_eff_merge <- mi_catch_eff_merge %>% 
  mutate(effort_ident = case_when(is.na(total_effort_1_effort) ~ NA,
                                  TRUE ~ effort_ident)) 

mi_catch_eff_merge %>% 
  group_by(is.na(age), is.na(total_effort_1_effort), is.na(effort_ident)) %>% 
  count()
#now we can see that aged fish have an NA effort ident becuase they don't have effort, there is also one odd survey that doesn't have effort that also does not retain an effort ident

#adding a flag for if a survey should be used for cpue generation or not
mi_catch_eff_merge %>% 
  group_by(flag) %>% 
  count() 

mi_catch_eff_merge %>% 
   group_by(flag, is.na(effort_ident)) %>% 
   count()

mi_catch_eff_merge <- mi_catch_eff_merge %>% 
  mutate(cpue_invalid = case_when(str_detect(flag, "no effort data") ~ "y",
                                  str_detect(flag, "likely") ~ "y",
                                  TRUE ~ "n"))
# all aged fish samples and no effort data samples are now told to be invaild for cpue

mi_catch_eff_merge %>% 
  group_by(cpue_invalid, flag, is.na(effort_ident)) %>% 
  count()

###cleaning up where we see na tei but still have nothing caught
mi_catch_eff_merge %>% 
  ungroup() %>% 
  summarise(problem = sum(is.na(effort_ident) & !is.na(nothing_caught)),
            na_ident = sum(is.na(effort_ident)),
            na_effort = sum(is.na(total_effort_1_effort)))

mi_catch_eff_merge %>% 
  filter(is.na(total_effort_1_effort)) %>% 
  group_by(nothing_caught) %>% 
  count()

mi_catch_eff_merge <- mi_catch_eff_merge %>% 
  ungroup() %>% 
  mutate(nothing_caught = case_when(is.na(effort_ident) ~ NA,
                                    TRUE ~ nothing_caught)) 
```                        

# Final clean up to match schema
```{r}
#matching to schema
