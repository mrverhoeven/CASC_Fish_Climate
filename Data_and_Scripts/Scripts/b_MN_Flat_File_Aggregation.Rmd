...
5. In 13 Aug cpue export, State ==NA, total effort ident = NA, Note that in the filtered cpe data file these records are NA-lade
...
7. Lake 28003200|1996|Trap has 2 idents one TEInothing caught=T and TEInothingcaught=F
9. Move DL's hiving code up into the actual data agg scripts
10.Chris Rounds is finding Fall EF dates that are actually reporting the lakefinder date for this survey rather than the actual observation date.
...
11.We have numerous TEIs with multiple total efforts. That's not how total effort works. Fix.
12.We have many TEIs tagged with nothing caught)
    ...
    Send all Nothing caught = True total efforts off to Jon Hansen
13. MOve lake area data over to the agg script (trims lots of NAs in other states)
14. Grace found whole surveys-lake-year that were excluded from cris custer cpe output but were present in lake finder 
...
15. Clean up sex column Male, Female, Unknown, NA (not reported, not sexed, data lost, all those things)
16. QC on age column
17. negative vals on sample ids (whats that mean and should they be removed?)

      #schema and hive match
18. Add sampling method simple column to hive
19. Fix units to match hive structure, make sure anything with na values are also na for the unit (e.g., units like net-nights -> net_nights)
20. Where values are NA, units should also be NA (efforts, lengths, weights, others?)
      
OUTSTANDING FIXES
21. Schema matched to overall hive (column names and column inclusions)
22. Check N species in a survey (is 44 spp a reasonable finding?)


      #flags
23. Check that our flagging is spelling out implications
24. Add flags for 0 efforts and high efforts
25. Auto Flagging: (build a flagging template from the MW Hive then apply it state-by-state)
...
#other notes  
  ...
  # workship results
  note_revisions <- fread("D:/Shared drives/Hansen Lab/RESEARCH PROJECTS/Fish Survey Data/MN_Data    Data_and_Scripts\\Data\\input\\MN_comments_notes_reviewed.csv")
