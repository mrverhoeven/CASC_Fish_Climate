...
3. Historical WAE survey duplications
  - first check the CPUE data connection to efforts-- there are a heap of issues with the "External Management Survey" (from effort) versus "External Mgmt Survey" (from indiv fish)
  - Consider dropping the effort table info where we have a HISTORICAL cpue record (or at least scope this strategy)
  - See line 1276 for Id of problem records. A band-aid is to drop them using the table. 
7. bring in the comments, save the notes field and add flags to the database records
  -   flags as Mike added, flag for representative smapling
  - where gear notes marked with dippers, bring back dipper # into end of gear desc. 
...
full_names <- c("Initial Survey", "Re-Survey", "Population Assessment", "Special Assessment", "Creel Survey", "Winter Kill Assessment", 
                ...
                "Investigational Survey", "Research Survey", "External Mgmt Survey", "External Research Survey", "Standard Survey", "Targeted Survey")
...
#we know that dates are a problem, can we schmooze that join? MEMORY HOG PINCH POINT
...
#now roll the catch CPUE data back to this bigger file:
  ...
  #subset of MN_cpue where lake/date/method/type are not matched:
...
flatfish <- readRDS("mn_flat_effort_indivfish_merge.rds")
...
#species names are mixed codes and names
    ...
    #adds names to that table (uses key provided by Jon Hansen, MNDNR on 10 Oct 2023: https://docs.google.com/spreadsheets/d/1ZakE4S2zfSdYNEWHfikFqILNnD1gYpNf?rtpof=true&usp=drive_fs )    
      ...
      mn_dat[species_codes_mn,
        ...
        #still 2724 NAs here (nothing caught surveyXgears)
...
#DELETE ME IF SURVEY ID IS NOT NEEDED BELOW 
...
#other notes  
  ...
  mn_dat[ , .N , notes ] #send to workshop
  ...
  mn_dat[ , .N , garbage_bin_notes.2_indivfish] #Workshop
...
area <- read_csv("mn_lake_list.csv") %>%
...
mn_renamer <- fread( file = "mn_renamer.csv") #read in 
...
write_dataset(dataset = mn_dat, path = "mn_file_arrow")
