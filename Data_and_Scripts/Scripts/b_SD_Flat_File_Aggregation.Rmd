...
1. Amend original file names in data explainer to recognize sheet from which each dataset came, carry these through to 
...
sd_merged_ce[sd_locs, on = .(lake_id = lake_id ) , `:=` ("lat_unspec" = lat_unspec, "lon_unspec" = lon_unspec) ]
...
#date
sd_merged_ce[ , date := as.IDate(date) , ]
...
sd_merged_ce[ , .N , sampling_method_2 ]
sd_merged_ce[ , sampling_method_2 := as.character(sampling_method_2) , ]

sd_merged_ce[sd_gears, on = .(sampling_method = old_gear), sampling_method_2 := new_gear]
...
#total effort review:
sd_data %>% 
  group_by(total_effort_ident,total_effort_1,total_effort_1_units, sampling_method, year(date), lake_name, lake_id) %>% 
  count() %>%
  collect()
