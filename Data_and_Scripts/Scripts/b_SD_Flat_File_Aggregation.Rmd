...
#3934 obs from these survey ids is the excate amount of obs within total effort 1 of the parquet
 #this grouping was erroneously created - grouped together due to the lack of data, i will remove them and proceed 
...
sd_merged_ce %>% 
  ...
  print(n = nrow(.))
#great agreement in naming across the board
...
sd_merged_ce_trial <- sd_merged_ce %>% 
  ...
  mutate(mean_effort = mean(total_effort_1, na.rm = TRUE),
         ...
         flag = case_when(
           total_effort_1 == 0 ~ paste("effort reported as 0, typical sets are for 1440 min but cannot be certain", ifelse(is.na(flag), "", flag)),
           total_effort_1 < 0 ~ paste("invalid effort (negative)", ifelse(is.na(flag), "", flag)),
           z_score > 10 ~ paste("high effort", ifelse(is.na(flag), "", flag)),
           TRUE ~ flag)) %>%
  ungroup() %>% 
  select(state, 
         county, 
