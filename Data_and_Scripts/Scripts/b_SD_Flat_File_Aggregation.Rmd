...
files_list <- list.files(path = "G:/Shared drives/Hansen Lab/RESEARCH PROJECTS/Fish Survey Data/SD_Data/sd_raw_disaggregated_data", pattern = ".+\\.csv") #grabs only.csv files
files_list
...
for(i in 1:n) {
  ...
  assign(filei ,
          fread(paste0("G:/Shared drives/Hansen Lab/RESEARCH PROJECTS/Fish Survey Data/SD_Data/sd_raw_disaggregated_data/",
                                          files_list[i]),
         drop = c("V1")))
...
sd_merged_ce %>% 
  filter(is.na(date.1)) %>% 
  group_by(survey_id, is.na(original_file_name.1effort), is.na(original_file_name.1indivfish)) %>% 
  count()
...
sd_merged_ce %>% 
  filter(total_effort_1_units == "unspecified") %>% 
  group_by(sampling_method.1, total_effort_1) %>% 
  count() %>% 
  print(n = nrow(.))
#seems like unspecified units for total effort is handeled okay here
...
sd_merged_ce %>% 
  mutate(effort_flag = case_when(str_detect(flag, "effort") ~ "y",
                                 TRUE ~ "n")) %>% 
  group_by(total_effort_ident) %>% 
  summarise(n = n_distinct(effort_flag)) %>% 
  filter(n >1)

#flag for fish
sd_merged_ce <- sd_merged_ce %>% 
  mutate(joining_id = row_number())
...
length_weight_flag_1 <- sd_merged_ce %>%
  ...
  mutate(
    ...
    mean_length = mean(length.1, na.rm = TRUE),
    ...
    flag_new = case_when(
      z_score_length >= 10 | z_score_length <= -10 ~ "unlikely length ; ",
      z_score_weight >= 10 | z_score_weight <= -10 ~ "unlikely weight ; ",
      length.1 == 0 | weight.1 == 0 ~ "length or weight reported as 0 ; ",
      TRUE ~ NA_character_
    )
  ) %>%
  ungroup() %>%
  select(joining_id, flag_new)

sd_merged_ce <- sd_merged_ce %>%
  left_join(length_weight_flag_1, by = "joining_id") %>%
  mutate(flag = paste0(coalesce(flag, ""), coalesce(flag_new, "")))

sd_merged_ce <- sd_merged_ce %>% 
  mutate(flag = case_when(flag == "" ~ NA,
                          TRUE ~ flag))
...
#clean up units
sd_merged_ce <- sd_merged_ce %>%
  mutate(length_unit.1 = case_when(length_unit.1 == "mm" ~ "millimeters",
                                   TRUE ~ length_unit.1),
         weight_unit.1 = case_when(weight_unit.1 == "g" ~ "grams",
                                   TRUE ~ weight_unit.1)) 

#fix aging structure
sd_merged_ce <- sd_merged_ce %>% 
  mutate(aging_structure.1 = case_when(aging_structure.1 == "Otolith" ~ "otolith",
                                       aging_structure.1 == "Scale" ~ "scale",
                                       aging_structure.1 == "dorsal spine" ~ "spine_dorsal",
                                       aging_structure.1 == "otollith" ~ "otolith",
                                       aging_structure.1 == "ototlith" ~ "otolith",
                                       aging_structure.1 == "pectoral spine" ~ "spine_pectoral",
                                       TRUE ~ aging_structure.1)) 
```
...
#2. ensure I am keeping the correct sampling method (see code below)
...
rm(dates)
#survey date is the first sampling within a lake-year (any gear within a lake)
...
sd_merged_ce_trial %>% 
  distinct(length_unit_1, weight_unit_1)
```
...
sd_merged_ce_trial <- sd_merged_ce_trial %>% 
...
sd_merged_ce_trial %>% 
  ...
  count()
#total of true matches halk obs 
