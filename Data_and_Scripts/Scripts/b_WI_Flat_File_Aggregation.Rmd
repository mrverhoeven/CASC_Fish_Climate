...
files_list <- list.files(path = "wi_raw_disaggregated_data/wi_raw_disaggregated_data", pattern = ".+\\.csv") #grabs only.csv files
files_list
...
for(i in 1:n) {
  ...
  assign(filei ,
          fread(paste0("wi_raw_disaggregated_data/wi_raw_disaggregated_data/",
                                          files_list[i])))
...
are the aged fish also in the wi_inland_fishobservations_10Nov2023 file? (MOVED TO ITS OWN SCRIPT, not run here to save memory & runtimes)
```{r}
        
#This is an issue: 

# 
#         # fish_ob_test <- wi_inland_fishobservations_10Nov2023[survey_id == "281" & sampling_method == "fyke_net" & species.1 == "walleye", .N ,][,.(survey_id, sampling_method, species.1, lake_id, length.1, date, age),]
#         # # 650 walleye in a fyke net in this survey
#         # 
#         # 
#         # len_age_test <- wi_inland_lenage_10Nov2023[survey_id == "281" & sampling_method == "fyke_net" & species.1 == "walleye",  .(survey_id, sampling_method, species.1, lake_id, length.1, date, age)]
...
# wi_inland_fishobservations_10Nov2023[survey_id == "281" & sampling_method == "fyke_net" & species.1 == "walleye", , ][,.(survey_id, sampling_method, species.1, lake_id, length.1, date, age),][order(length.1)][ ,.N , length.1]
#   wi_inland_fishobservations_10Nov2023[survey_id == "281" & sampling_method == "fyke_net" & species.1 == "walleye", , ][,.(survey_id, sampling_method, species.1, lake_id, length.1, date, age),][order(length.1)]
# 
# wi_inland_lenage_10Nov2023[survey_id == "281" & sampling_method == "fyke_net" & species.1 == "walleye",  .(survey_id, sampling_method, species.1, lake_id, length.1, date, age)][order(length.1)][ ,.N , length.1]
#   wi_inland_lenage_10Nov2023[survey_id == "281" & sampling_method == "fyke_net" & species.1 == "walleye",  .(survey_id, sampling_method, species.1, lake_id, length.1, date, age)][order(length.1)]
# 
#   wi_inland_lenage_10Nov2023[ , date := as.IDate(date) , ]
#   
# crosscheck <- merge(
#   wi_inland_fishobservations_10Nov2023[ , .("obsN" = .N), .(survey_id, sampling_method, species.1, lake_id, length.1, date) ],
#   wi_inland_lenage_10Nov2023[ , .("laaN" = .N), .(survey_id, sampling_method, species.1, lake_id, length.1, date) ],
...
# wi_inland_lenage_10Nov2023[,.N ,(backcalculated_age)] #yes, dp, fl
# 
# #how many ages are NAs?
# wi_inland_lenage_10Nov2023[, .N,is.na(age)] #443,811 fish have ages and 239,785 fish do not have ages (35% of ages are NA)
# 
...
# 
# crosscheck <- merge(
#   wi_inland_fishobservations_10Nov2023[ , .("obsN" = .N), .(survey_id, sampling_method, species.1, lake_id, date) ],
#   wi_inland_lenage_10Nov2023[!is.na(age) & !backcalculated_age %in% c("dp", "yes", "fl")  , .("laaN" = .N), .(survey_id, sampling_method, species.1, lake_id, date) ],
...
# 
# 
# a <- wi_inland_fishobservations_10Nov2023[survey_id == 651 & species.1 == "yellow_perch"]
# 
# b <- wi_inland_lenage_10Nov2023[survey_id == 651 & species.1 == "yellow_perch"]
# 
# 
# 
# crosscheck <- merge(
#   wi_inland_fishobservations_10Nov2023[ , .("obsN" = .N), .(survey_id, species.1, lake_id) ],
#   wi_inland_lenage_10Nov2023[!is.na(age) & !backcalculated_age %in% c("dp", "yes", "fl")  , .("laaN" = .N), .(survey_id, species.1, lake_id) ],
...
wi_inland_fishobservations_10Nov2023[ , unique(sampling_method)  , ]
wi_inland_lenage_10Nov2023[ ,unique(sampling_method)  ,]


wi_inland_lenage_10Nov2023[ , .N , species]
...
wi_inland_fishobservations_10Nov2023[crosscheck, on = .(species, survey_id), flag := ifelse(laa_exceeds_obs==T, "Number of aged fish exceeds number of observed fish", "")  ]
...
#prep date col
wi_inland_lenage_10Nov2023[ , date := as.IDate(date,format = "%m/%d/%Y"),]
...
wi_inland_fishobservations_10Nov2023[ , id_indv_win_len := seq_len(.N) , .(lake_id, date, sampling_method, survey_id, species, length) ] #assign a id to each ind fish observed within a given length
wi_inland_lenage_10Nov2023[ , id_indv_win_len := seq_len(.N) , .(lake_id, date, sampling_method, survey_id, species, length) ]  #assign a id to each ind fish observed within a given length

#drop unneeded cols from age data
wi_inland_lenage_10Nov2023 <- wi_inland_lenage_10Nov2023[ , .(lake_id, date, sampling_method, survey_id, species, length, weight, age, aging_structure, sex, reproductive_condition_notes, aging_data_notes, id_indv_win_len, original_file_name) , ]
#drop unneeded cols from indivfish table
wi_inland_fishobservations_10Nov2023[ , age := NULL , ]

#cleanup ws to prep for heavy merge
rm(cde, crosscheck, gear_table, wi_inland_cpue_19Mar2021, cols, filei, files_list, i, maxn, n)
#merge/do left join 
wi_inland_fishobservations_10Nov2023 <- 
...
wi_inland_fishobservations_10Nov2023[  , .N  , .("Has_Observatinon_Dat" = !is.na(original_file_name_indivfish), "Has_age_dat" = !is.na(age))   ]
...
wi_inland_fishobservations_10Nov2023[ , .N , .(is.na(date), is.na(year)) ] #no cases of na date where year can cover, drop year
wi_inland_fishobservations_10Nov2023[ , year := NULL , ]
...
#survey_type
colnames(wi_inland_fishobservations_10Nov2023)[str_detect(colnames(wi_inland_fishobservations_10Nov2023), "survey_type.1")] #what's here for this var name?
#consolidate if possible
wi_inland_fishobservations_10Nov2023[ survey_type.1_indivfish != survey_type.1_effort , .N ,  ] #any disagreememnt among non-NA fields
...
wi_target_cisco8nov2023 <- read_csv("wi_raw_disaggregated_data/wi_target_cisco8nov2023.csv")
...
rm(cisco,nothing_caught,wi_cisco_clean,wi_cisco_clean_uncount,wi_cisco_one_fish_per_row, wi_target_cisco8nov2023)
...
wi_cisco_final[ , c("survey_effort", "waterbody_type", "notes_1", "garbage_bin_notes_1") := NULL , ]

setnames(wi_cisco_final,
         old = c("survey_type",
                 "target_species",
                 "sampling_method_2",
                 "total_effort_1",
...
wi_inland_fishobservations_10Nov2023[, original_file_names :=  paste(original_file_names, original_file_name), ]
...
#but for winnebago, we actually have sub_effort.1 (effort_ident is a sample out of a single net on winnebago on a date) Fix that:
  wi_inland_fishobservations_10Nov2023[ !str_detect(original_file_names, "winnebago") , length(unique(effort_ident))  , .(date )  ]
  #DEPRECATED 5Feb 2024 wi_inland_fishobservations_10Nov2023[ !str_detect(original_file_names, "winnebago"), sub_effort_ident := effort_ident , ]
...
#build a grouping var for the total effort (gearxdatexwinnebago)  
  ...
  wi_inland_fishobservations_10Nov2023[ str_detect(original_file_names, "cisco") , sample_id := .GRP + (wi_inland_fishobservations_10Nov2023[ , max(sample_id, na.rm = T) ,])  , .(date, sampling_method,lake_id) ] #start at the current sample_id.1 value
  
  wi_inland_fishobservations_10Nov2023[ , summary(sample_id) , ]
  
  wi_inland_fishobservations_10Nov2023[is.na(sample_id), .N , .(original_file_name, original_file_names) ]
  
  
#now roll the rest of the dataset's sample_ids into that field
  # wi_inland_fishobservations_10Nov2023[ is.na(total_effort_ident), .N , ]
  wi_inland_fishobservations_10Nov2023[, `:=` (effort_ident = NULL) ,  ]

#does that effort grouper work for total eff 1 & 2 ?
wi_inland_fishobservations_10Nov2023[ , length(unique(total.effort.1)) , .(lake_id, lake_name, date, sampling_method, sample_id, total.effort.1) ][ , .N , V1] #each eff gets own row
...
wi_inland_fishobservations_10Nov2023[ , .N , ]

wi_inland_fishobservations_10Nov2023[ ,`:=` (total_effort_ident = .GRP)  , .(lake_id, lake_name, date, sampling_method) ]

#any no catch data?
  wi_inland_fishobservations_10Nov2023[ , sum(!is.na(species)) , .(sample_id) ] #yes! every sub_effort_ident with a zero here is a no catch
...
new <-  c("sub_effort_1", "sub_effort_1_units","total_effort_ident","sub_effort_2","sub_effort_2_units")
...
# this correction is deprecated
      # #all winnebago sub_effort data are contained in the total effort column (will need to calc new total effort vals)
      # wi_inland_fishobservations_10Nov2023[str_detect(lake_name,"innebago"), .N , .(total_effort_ident,sub_effort_ident) ]
      # 
      # 
      # #for winnebago, total effort from a survey is a sum of how many 5 minute trawls were on that date
      # wi_inland_fishobservations_10Nov2023[ str_detect(original_file_names, "winnebago") ,
      #                                       .("n_uniquesubidents" = length(unique(sub_effort_ident)), "unique_eff" = length(unique(sub_effort_1))) , .(total_effort_ident, sub_effort_ident, date, sampling_method, total_effort_1, total_effort_1_unit) ] #these are all 1 unit of 5 minute trawl or 1 net-night of gillnet
      # 
      # 
      # #calculate trawl efforts
      # wi_inland_fishobservations_10Nov2023[  str_detect(original_file_names, "winnebago") & sampling_method == "trawl" , `:=` (total_effort_1 = length(unique(sub_effort_ident))*5,
      #                                                                                                       total_effort_1_unit = "minutes"),
      #                                                                                                       .(total_effort_ident) ]
      # 
      # #calculate gillnet efforts
      # wi_inland_fishobservations_10Nov2023[ str_detect(original_file_names, "winnebago")  & sampling_method == "gillnet" , `:=` (total_effort_1 = length(unique(sub_effort_ident)),
      #                                                                                                       total_effort_1_unit = "net-nights"),
      #                                                                                                       .(total_effort_ident) ]
      # 
      # wi_inland_fishobservations_10Nov2023[ !is.na(total_effort_ident) , .("nfish" = .N, unique(total_effort_1), unique(total_effort_1_unit)) , .(total_effort_ident, sampling_method)]
      #   
#  
...
# 
# 
# setcolorder(wi_inland_fishobservations_10Nov2023, not_garbage) #reorder from this list before axe-ing the garbage cols
# 
# wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023[ , ..not_garbage , ]
# 
...
```{r}
# add NHDids
MGLP_crosswalk <- fread("MGLP_FISH_LAKES_12Aug24.csv")
...
wi_lat_long <- read_csv("wi_raw_disaggregated_data/wi_lake_wbic_lat_long_PNF.csv")%>%
...
WI_hydro_list <- read_csv("wi_raw_disaggregated_data/24k_Hydro_Waterbodies_(Open_Water).csv")
