...
DONE  13. cleannames() to colnames
      ...
      17. Ensure nothing_caught is asisgned at the total_effort_ident level
      ...
      22. May lengthed fish do not have a unit on their length
...
WI_dat_with_hydrotype <-wi_inland_fishobservations_19Mar2021 %>% 
...
wi_lake_names <- wi_inland_fishobservations_19Mar2021 %>%
  group_by(lake_id)%>%
  collect()%>%
...
wi_filtered <- wi_inland_fishobservations_19Mar2021 %>% 
...
#cutting the fish as rows data frame by the object filtered only to contain lakes
wi_inland_fishobservations_19Mar2021 <- wi_inland_fishobservations_19Mar2021 %>% 
  right_join(wi_filtered)
