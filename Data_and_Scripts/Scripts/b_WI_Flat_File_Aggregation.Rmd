...
# 6/25/25 - exploration of effort based on concerns
glimpse(wi_inland_cpue_19Mar2021)
wi_inland_cpue_19Mar2021 %>% 
  group_by(sampling_method, cpue_units) %>% 
  count()

cpe <- wi_inland_cpue_19Mar2021 %>% 
  filter(str_detect(lake_name, "lake_chippewa") & year == "1990" & sampling_method == "netting") %>% 
  glimpse()

chip_effort <- wi_inland_effort_10Nov2023 %>% 
  filter(str_detect(lake_name, "lake_chippewa") & year == "1990" & sampling_method == "fyke_net" & survey_id == "41711425")  %>% 
  select(lake_name,
         year,
         date,
         target_species,
         ...
         sample_time_notes.3,
         gear_data_notes.9) %>% 
  mutate(number_of_nets = str_extract(gear_data_notes.9, "\\d+")) %>% 
  mutate(number_of_nets = as.numeric(number_of_nets))

chip_effort %>% 
  group_by(target_species) %>% 
  summarise(total = sum(number_of_nets))
#yikes, looks like we will need to separate total effort generated by target species
rm(chip_effort, cpe)

wi_inland_effort_10Nov2023 %>% 
  filter(str_detect(sampling_method, "net")) %>% 
  filter(!(str_detect(gear_data_notes.9, "NA"))) %>% 
  mutate(number_of_nets = str_extract(gear_data_notes.9, "\\d+"),
         hours_set = as.numeric(str_extract(sample_time_notes.3, "\\d+"))) %>% 
  mutate(number_of_nets = as.numeric(number_of_nets),
         hours_set = as.numeric(hours_set)) %>% 
  mutate(night_check = hours_set/24,
         is_whole = case_when(night_check %% 1 == 0 ~ "y",
                              TRUE ~ "n")) %>% 
  summarise(prop_whole = sum(is_whole == "y", na.rm = T)/sum(is_whole == "y" | is_whole == "n", na.rm = T),
            prop_1 = sum(night_check == 1, na.rm = T)/sum(is_whole == "y"))
#88% of netting surveys are whole numbers lending to net nights
#90% of the whole number are one net night 
wi_inland_effort_10Nov2023 %>% 
  filter(str_detect(sampling_method, "net")) %>% 
  filter(!(str_detect(gear_data_notes.9, "NA"))) %>% 
  mutate(number_of_nets = str_extract(gear_data_notes.9, "\\d+"),
  ...
  count()
#most stem from bottom_gill_nets and fyke nets
...
wi_inland_effort_10Nov2023 %>% 
  ...
  mutate(number_of_nets = as.numeric(number_of_nets),
         hours_set = as.numeric(hours_set),
         net_hours = number_of_nets * hours_set,
         net_nights = net_hours/ 24) 

wi_inland_effort_10Nov2023 %>% 
  filter(!str_detect(gear_data_notes.9, "NA")) %>% 
  filter(!str_detect(sample_time_notes.3, "NA")) %>% 
  group_by(sampling_method) %>% 
  count() 
...
gear_table[sampling_method %in% c("dip_net", "hand_net"), simple_gear := "other"]
...
gear_table %>% 
  group_by(simple_gear, sampling_method) %>% 
  count() %>% 
  arrange(simple_gear) %>% 
  print(n = nrow(.))
...
wi_inland_effort_10Nov2023 <- wi_inland_effort_10Nov2023 %>%
  ...
  mutate(number_of_nets = as.numeric(number_of_nets),
         hours_set_net = as.numeric(hours_set_net),
         net_hours = hours_set_net*number_of_nets,
         net_nights = net_hours/24,
         is_divisible_by_24 = if_else(hours_set_net %% 24 == 0, "y", "n")) %>% 
  mutate(net_nights = as.character(net_nights),
         hours_set_net = as.character(hours_set_net),
         number_of_nets = as.character(number_of_nets)) %>% 
  mutate(total.effort.1 = case_when(simple_gear == "electrofishing" ~ sample_time_min, #create total effort 1
                                    simple_gear == "netting" & is_divisible_by_24 == "y" ~ net_nights,
                                    simple_gear == "netting" & is_divisible_by_24 == "n" ~ hours_set_net,
  ...
  mutate(total.effort.2 = case_when(simple_gear == "electrofishing" ~ distance, #create total effort 2 if two relevant efforts
                                    simple_gear == "netting" & is_divisible_by_24 == "n" ~ number_of_nets,
  ...
  mutate(total.effort.1.units = case_when(simple_gear == "electrofishing" ~ "minutes", #assign units to efforts in effort 1
                                    simple_gear == "netting" & is_divisible_by_24 == "y" ~ "net_nights",
                                    simple_gear == "netting" & is_divisible_by_24 == "n" ~ "hours",
...
wi_inland_effort_10Nov2023 %>% 
  group_by(simple_gear, sampling_method, total.effort.1.units, total.effort.2.units) %>% 
  count() %>% 
  arrange(simple_gear, sampling_method) %>% 
  print(n = nrow(.))

wi_inland_effort_10Nov2023 %>% 
  filter(total.effort.1.units == "net_nights") %>%
  mutate(total.effort.1 = as.numeric(total.effort.1)) %>% 
  summarise(has_dec = any(total.effort.1 %% 1 != 0))
```
...
glimpse(wi_winnebago_cpue_23Apr2021)
wi_winnebago_cpue_23Apr2021 %>% 
  filter(date == "2014-08-06") %>% 
  filter(species == "walleye") %>% 
  distinct(site_id.1, site_id.2, site_id.3, site_id.4, sampling_method, cpue)
...
wi_winnebago_fishobservations_23April2021 %>%
  filter(year(date) == "2014" & sampling_method == "trawl" & month(date) == "8" & day(date) == "6") %>% 
  group_by(site_id.1, site_id.2, site_id.3, site_id.4, species, date, gear_data_notes, sampling_method) %>% 
  count() %>% 
  ungroup() %>% 
  distinct(site_id.1, site_id.2, site_id.3, site_id.4) %>% 
  print(n = nrow(.))
...
glimpse(wi_winnebago_fishobservations_23April2021)

#based on the original date - looks like site.id is 1.) area 2.) grid 3.) station 4.) circuit
...
wi_inland_lenage_10Nov2023 %>% 
  mutate(year_check = year(date)) %>% 
  group_by(year, year_check) %>% 
  count() %>% 
  arrange(year_check) %>% 
  print(n = nrow(.))
# we can see here there are some funky dates from this file but year is preserved 
```
...
# so a quick recap here is that there are ~20k records in the fish obs that do not have a sample date a person could dredge up the survey start and end dates, but it is not clear to me how that could help ID fish within a survey. For another 3.5k, there are bad dates in the length age data 
...
wi_inland_fishobservations_10Nov2023 %>% 
  filter(lake_id == "2399700" & year(date) == "1990" & sampling_method == "fyke_net") %>% 
  distinct(survey_id,
           sample_id,
    total_effort, 
         total.effort.1,
...
wi_inland_fishobservations_10Nov2023 %>% 
  filter(total.effort.1.units == "number_of_nets" &
            sampling_method %in% c("frame_net_miniature", 
                             "fyke_net", "gill_net_bottom", 
                             "gill_net_vertical", 
                             "gill_net_floating")) %>% 
  group_by(lake_name, lubridate::year(date)) %>% 
  count() %>% 
  print(n = nrow(.))

# this notes is made on 6/26/25 - below had previously converted number of nets to gill nets. I have since vetted the total effort and calculated night nets in the effort prep chunk. We cannot just convert to net_nights because we can't assume the duration that they were sent 
...
wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(lake_name, "innebag") & date == "2014-08-06") %>% 
  distinct(effort_ident, sample_id, total.effort.1, site_id.1, site_id.2, site_id.3, site_id.4) %>% 
  print(n = nrow(.))
#previously there was a creation of "effort ident" that included species in the grouping variable of the winnebago cpe data - this created several effort idents within a site (one for each species that was captured)
#also below is code that creates a sample id based on date and sampling method - that doesn't appear to be correct either... I think each site is more representative of a sample_id - lets add that to the grouping
#when looking below this is totally the case - total effort columns are transitioned into sub effort columns and sample_id gets shifted to sub effort ident. The old grouping of just date in winne makes it look like there is just a total effort of 5 for a given day when it is really 15*5. Thus, lets include site id as the sample id for these 

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(site_group = paste0(site_id.1, site_id.2, site_id.3, site_id.4))

wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(lake_name, "innebag") & date %in% c("2014-08-06", "2014-08-07")) %>% 
  group_by(date, sampling_method, lake_id) %>% 
  mutate(check = cur_group_id()) %>% 
  ungroup() %>% 
  distinct(site_group, check, sample_id, total.effort.1, site_id.1, site_id.2, site_id.3, site_id.4) %>% 
  print(n = nrow(.))
#this shows that the previous grouping of date and sampling method isn't specific enough for winnebago - it needs to be at the site level 

#build a grouping var for the total effort (gearxdatexwinnebago)  
  wi_inland_fishobservations_10Nov2023[ str_detect(original_file_names, "winnebago") , sample_id := .GRP + (wi_inland_fishobservations_10Nov2023[ , max(sample_id, na.rm = T) ,])  , .(date, sampling_method,lake_id, site_group) ] #start at the current sample_id.1 value
  ...
  #check winne sample id generation 
  wi_inland_fishobservations_10Nov2023 %>% 
    filter(str_detect(lake_name, "innebag") & date %in% c("2014-08-06", "2014-08-07")) %>% 
    distinct(date, site_group, sample_id, total.effort.1) %>% 
    print(n = nrow(.))
  #now we can see that we get a sample id for each site in winne
...
#quick check on winnebago data grouping
wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(lake_name, "innebag") & date %in% c("2014-08-06", "2014-08-07")) %>% 
  distinct(total_effort_ident, sample_id, total.effort.1, site_id.1, site_id.2, site_id.3, site_id.4)
...
#quick check on winnebago data grouping again
wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(lake_name, "innebag") & date %in% c("2014-08-06", "2014-08-07")) %>% 
  distinct(total_effort_ident, sub_effort_ident, total_effort_1, sub_effort_1)
...
wi_inland_fishobservations_10Nov2023[lubridate::year(date) < 1938 |lubridate::year(date) > 2023,unique(date)]

#first clean up flags then add new flag note
wi_inland_fishobservations_10Nov2023[ , .N , flag]
wi_inland_fishobservations_10Nov2023[lubridate::year(date) < 1938 |
                                        lubridate::year(date) > 2023 , flag := ifelse(is.na(flag), "bad_date" , paste(flag, "bad_date", sep = ";" )) ,]

wi_inland_fishobservations_10Nov2023 %>% 
  filter(lubridate::year(date) <1938 | lubridate::year(date) > 2023) %>% 
  distinct(date, flag)

#check winne catch as a gut check
wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(lake_name, "innebag") & date == "2014-08-06") %>% 
  group_by(total_effort_ident, total_effort_1) %>% 
  summarise(n_walleye = sum(species == "walleye"))

wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(lake_name, "innebag") & date == "2014-08-06") %>% 
  group_by(sub_effort_ident, sub_effort_1) %>% 
  summarise(n_walleye = sum(species == "walleye")) %>% 
  ungroup() %>% 
  summarise(n_effort = sum(sub_effort_1),
            n_walleye = sum(n_walleye))

#do we also need to eliminate the sub effort ident?
wi_inland_fishobservations_10Nov2023 %>% 
  summarise(n_no_tei = sum(is.na(total_effort_ident) & !is.na(sub_effort_ident)),
            no_effort = sum(is.na(total_effort_ident) & !is.na(sub_effort_ident) & is.na(total_effort_1)),
            no_sub_effort = sum(is.na(total_effort_ident) & !is.na(sub_effort_ident) & is.na(sub_effort_1)))
#we can see here that we removed the total effort ident for where there was no reported total effort, we now must do the same thing at the sub effort level 
check <- wi_inland_fishobservations_10Nov2023 %>% 
  filter(is.na(total_effort_1)) %>% 
  distinct(lake_name, date, total_effort_ident, total_effort_1, sub_effort_ident, sub_effort_1)

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(sub_effort_ident = case_when(is.na(sub_effort_1) ~ NA,
                                      TRUE ~ sub_effort_ident))
wi_inland_fishobservations_10Nov2023 %>% 
  summarise(n_no_tei = sum(is.na(total_effort_ident) & !is.na(sub_effort_ident)),
            no_effort = sum(is.na(total_effort_ident) & !is.na(sub_effort_ident) & is.na(sub_effort_1)),
            sub_effort_no_total = sum(is.na(total_effort_ident) & !is.na(sub_effort_ident) & !is.na(sub_effort_1)))
#we can see where, we have 40751 cases where total effort ident is NA due to missing total effort but a sub effort ident remains - non of the sub effort ident cases remaining have na sub effort and all of the cases where a total effort ident is missing but has a sub effort ident remaining is due to there being sub effort in one part but not the other

wi_inland_fishobservations_10Nov2023 %>% 
  summarise(no_id_tf = sum(is.na(total_effort_ident) & !is.na(total_effort_nothing_caught)),
            no_id_n = sum(is.na(total_effort_1)),
            no_sub_tf = sum(is.na(sub_effort_ident) & !is.na(sub_nothing_caught)),
            no_sub_n = sum(is.na(sub_effort_1)))
#we can see here places where the effort idents are NA but the nothing caught columns are not - in all cases it is due to the effort being NA

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(total_effort_nothing_caught = case_when(is.na(total_effort_ident) ~ NA,
                                                 TRUE ~ total_effort_nothing_caught),
         sub_nothing_caught = case_when(is.na(sub_effort_ident) ~ NA,
                                        TRUE ~ sub_nothing_caught)) 
```
...
wi_inland_fishobservations_10Nov2023 %>% 
  ...
  summarise(tags = sum(str_detect(nhdhr_id, "nhdhr")),
            total = n())
#found some records in Wisconsin that do not pick up an nhdhr_id tag

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(nhdhr_id = case_when(!(str_detect(nhdhr_id, "nhdhr")) ~ paste0("nhdhr_", nhdhr_id),
                              TRUE ~ nhdhr_id))
```
...
wi_inland_fishobservations_10Nov2023[ is.na(year), .N , is.na(date) ]
  wi_inland_fishobservations_10Nov2023[ is.na(year) & !str_detect(flag, "bad_date"), year := lubridate::year(date) ] #this fills year when it is na and date doesn't have a flag
  
#dates
wi_inland_fishobservations_10Nov2023[ !str_detect(flag, "bad_date"), month := lubridate::month(date) , ]
...
wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(flag, "bad_date")) %>% 
  group_by(total_effort_ident, date) %>% 
  count() %>% 
  print(n = nrow(.))
#all bad dates do not have a total effort ident, in the chunk below, all na total effort idents will get the first record of date within na total effort idents. Let's only apply this ordering to records that have a total effort ident

wi_inland_fishobservations_10Nov2023[ !is.na(total_effort_ident), date_total_effort_ident := date[order(date)][1], by = total_effort_ident]

wi_inland_fishobservations_10Nov2023 %>% 
  group_by(total_effort_ident) %>% 
  summarise(n_tei_dates = n_distinct(date_total_effort_ident)) %>% 
  filter(n_tei_dates >1)

wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(flag, "bad_date")) %>% 
  group_by(total_effort_ident, date_total_effort_ident) %>% 
  count() %>% 
  print(n = nrow(.))
...
#doing the same thing as above, only taking the first date where total effort ident is not NA
wi_inland_fishobservations_10Nov2023[!is.na(total_effort_ident), 
                                     date_survey := min(date, na.rm = T), 
                                     by = survey_id] 
...
wi_inland_fishobservations_10Nov2023[!is.na(sub_effort_ident), 
  date_sub_effort_ident := min(date, na.rm = T), 
  by = sub_effort_ident]

wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(flag, "bad_date")) %>%
  group_by(sub_effort_ident, date_sub_effort_ident) %>% 
  count()
...
wi_inland_fishobservations_10Nov2023[, site_id := do.call(paste, c(replace(.SD, is.na(.SD), ""), sep = "")), .SDcols= c("site_id",
                                                                                                   ...
                                                                                                   "site_id_3",
                                                                                                   "site_id_4")]
wi_inland_fishobservations_10Nov2023[ ,sort(unique(site_id)) ,]

wi_inland_fishobservations_10Nov2023[ site_id == "", site_id := NA]
...
#age class -- not really, moving these to notes
  wi_inland_fishobservations_10Nov2023[ , .N , .(reproductive_condition_notes) ]
  wi_inland_fishobservations_10Nov2023[str_detect(reproductive_condition_notes, ":NA") , reproductive_condition_notes := NA,  ]
  
  wi_inland_fishobservations_10Nov2023[ ,  .N ,  aging_data_notes]
  wi_inland_fishobservations_10Nov2023[str_detect(aging_data_notes, ":NA") , aging_data_notes := NA ]
  wi_inland_fishobservations_10Nov2023[!is.na(aging_data_notes), aging_data_notes := paste("age.note.", aging_data_notes, sep = ""), ]
  
  wi_inland_fishobservations_10Nov2023[ , .N , notes ]
  wi_inland_fishobservations_10Nov2023[str_detect(notes, ":NA"), .N , notes ]
  wi_inland_fishobservations_10Nov2023[str_detect(notes, ":NA") , notes:=NA,  ]
  
#note
wi_inland_fishobservations_10Nov2023[ , .N , notes]
wi_inland_fishobservations_10Nov2023[, notes := do.call(paste, c(replace(.SD, is.na(.SD), ""), sep = "; ")), .SDcols= c("aging_data_notes", "reproductive_condition_notes" ,"notes")]

wi_inland_fishobservations_10Nov2023[  ,c("aging_data_notes", "reproductive_condition_notes", "id_indv_win_len", "individual_count") := NULL , ]  
  
#drop some other shit (these are ~5377 fish whose efofrts could maybe be recovered if important to someone)
wi_inland_fishobservations_10Nov2023[is.na(total_effort_1) & is.na(total_effort_2) , .N , .(effort, effort_units, number_of_nets, hours_set_net, distance, distance_units) ]

wi_inland_fishobservations_10Nov2023[  ,c("id_indv_win_len", "individual_count", "effort_ident","effort", "effort_units", "number_of_nets", "hours_set_net", "distance", "distance_units") := NULL , ] 

wi_inland_fishobservations_10Nov2023[ , .N , .(garbage_bin_notes_1_effort, garbage_bin_notes_2_effort) ]

wi_inland_fishobservations_10Nov2023[  ,c("garbage_bin_notes_1_effort", "garbage_bin_notes_2_effort", "hydrotype" , "simple_gear", "unique_row_key", "data_type", "source_agency", "source_contact", "jan2023_inventoried") := NULL , ] 

#age_class

wi_inland_fishobservations_10Nov2023[ , .N  , young_of_year ]
wi_inland_fishobservations_10Nov2023[young_of_year %in% c("y", "Y"), age_class := "young_of_year"]
wi_inland_fishobservations_10Nov2023[young_of_year %in% c("n", "N", "n."), age_class := "not_young_of_year"]
wi_inland_fishobservations_10Nov2023[young_of_year %in% c("", "b"), age_class := NA]
wi_inland_fishobservations_10Nov2023[ , .N  , age_class ]
wi_inland_fishobservations_10Nov2023[ , young_of_year:= NULL , ]


#observation ident assignment
wi_inland_fishobservations_10Nov2023[ , obs_id := .I , ]
```
...
new_colnames <- c(
  ...
  "date_sub_effort_ident" = "date_sub_effort_ident",
  ...
  "water_clarity_units" = "water_clarity_units",
  "lat_start" = "lat_start",
  "lon_start" = "lon_start",
  "lat_end" = "lat_end",
  "lon_end" = "lon_end",
  "site_id" = "site_id",
  "sub_effort_ident" = "sub_effort_ident",
  "sub_effort_1" = "sub_effort_1",
  "sub_effort_1_units" = "sub_effort_1_units",
  "sub_effort_2" = "sub_effort_2",
  "sub_effort_2_units" = "sub_effort_2_units",
  "sub_nothing_caught" = "sub_effort_nothing_caught",
  "species" = "species_1",
  "length" = "length_1",
  "length_unit" = "length_unit_1",
  "age" = "age",
  "aging_structure" = "aging_structure_1",
  "weight" = "weight_1",
  "weight_unit" = "weight_unit_1",
  "sex" = "sex",
  "flag" = "flag",
  "original_file_names" = "original_file_names",
  "lat_unspec" = "lat_unspec",
  "lon_unspec" = "lon_unspec",
  "waterbody_type" = "waterbody_type",
  "notes" = "notes_1"
)

setnames(wi_inland_fishobservations_10Nov2023, old = names(new_colnames), new = new_colnames)

wi_inland_fishobservations_10Nov2023[ , c("survey_type_2", "survey_type_3", "survey_type_4", "length_bin", "length_bin_unit",  "batch_weight", "batch_weight_unit", "ind_fish_ident", "lakesize", "lakesize_units", "location_notes_1" ) := NA]

# setdiff(names(wi_inland_fishobservations_10Nov2023), names(mw_data))
# setdiff(names(mw_data),names(wi_inland_fishobservations_10Nov2023) )

schema_names <- c(
  "state", "county", "lake_name", "lake_id", "nhdhr_id",
  "latitude_lake_centroid", "longitude_lake_centroid", "date_survey", "date_total_effort_ident",
  "date_sub_effort_ident", "date_sample", "year", "month", "survey_id", "survey_type",
  ...
  "total_effort_ident", "total_effort_1", "total_effort_2", "total_effort_1_units",
  "total_effort_2_units", "total_effort_nothing_caught", "water_temp", "water_temp_units",
  "water_clarity", "water_clarity_units", "lat_start", "lon_start", "lat_end", "lon_end",
  "site_id", "sub_effort_ident", "sub_effort_1", "sub_effort_1_units", "sub_effort_2",
  "sub_effort_2_units", "sub_effort_nothing_caught", "species_1", "length_1",
  "length_unit_1", "length_bin", "length_bin_unit", "age", "aging_structure_1", "weight_1",
  "weight_unit_1", "batch_weight", "batch_weight_unit", "sex", "age_class", "flag",
  "original_file_names", "ind_fish_ident", "lakesize", "lakesize_units", "lat_unspec",
  "lon_unspec", "waterbody_type", "location_notes_1", "notes_1", "obs_id"
)

setcolorder(wi_inland_fishobservations_10Nov2023, neworder = schema_names)
...
chr_cols <- c("state", "county", "lake_name", "lake_id", "nhdhr_id", "survey_id", "survey_type",
  ...
  "sub_effort_1_units","sub_effort_2_units","species_1","length_unit_1", "length_bin", "length_bin_unit","aging_structure_1",
   "weight_unit_1", "batch_weight", "batch_weight_unit", "sex", "age_class", "flag",
  "original_file_names", "ind_fish_ident","lakesize_units", "waterbody_type", "location_notes_1", "notes_1", "obs_id")

wi_inland_fishobservations_10Nov2023[, (chr_cols) := lapply(.SD, as.character), .SDcols = chr_cols]
...
num_cols <- c("latitude_lake_centroid", "longitude_lake_centroid" , "year", "month", "total_effort_1", "total_effort_2",
              ...
              "length_1", "age", "weight_1", "lakesize", "lat_unspec", "lon_unspec")

wi_inland_fishobservations_10Nov2023[, (num_cols) := lapply(.SD, as.numeric), .SDcols = num_cols]



#notes
wi_inland_fishobservations_10Nov2023[ , .N , notes_1 ]
wi_inland_fishobservations_10Nov2023[ !is.na(notes_1) , notes_1 := gsub("^\\s*;+|;+\\s*$", "", gsub(";\\s*;+", ";", notes_1)) ]
# wi_inland_fishobservations_10Nov2023[ , notes := NULL]
...
flag_xwalk <- wi_inland_fishobservations_10Nov2023 %>% 
  ...
  mutate(mean_effort = mean(total_effort_1, na.rm = TRUE),
         ...
         flag = case_when(total_effort_1 < 0 ~ paste("invalid effort (negative)", coalesce(flag, "")),
                          ...
                          is.na(date_total_effort_ident) & !is.na(total_effort_ident) ~ paste("no date reported", ";", coalesce(flag, "")),
...
flag_xwalk_2 <- wi_inland_fishobservations_10Nov2023 %>% 
  distinct(total_effort_ident, .keep_all = TRUE) %>%
...
wi_inland_fishobservations_10Nov2023[flag_xwalk_effort , on = .(total_effort_ident = total_effort_ident), tei_flag:= tei_flag ]
wi_inland_fishobservations_10Nov2023[ , .N , .(tei_flag) ]
wi_inland_fishobservations_10Nov2023[str_detect(tei_flag, " ; ") , .N ,tei_flag]
wi_inland_fishobservations_10Nov2023[ , tei_flag := gsub(" ; ", "", tei_flag) , ]

wi_inland_fishobservations_10Nov2023[, flag := do.call(paste, c(replace(.SD, is.na(.SD), ""), sep = "; ")), .SDcols= c("flag", "tei_flag")]

# Remove "; " from the beginning or end
wi_inland_fishobservations_10Nov2023[!is.na(flag), flag := gsub("^\\s*;+|;+\\s*$", "", gsub(";\\s*;+", "; ", flag))]
...
length_stats <- wi_inland_fishobservations_10Nov2023[, .(stat = c(names(summary(length_1)), "sd"),
...
#zero lengths
wi_inland_fishobservations_10Nov2023[length_1 == 0 , len_flag := "length reported as zero"]
...
wi_inland_fishobservations_10Nov2023[length_stats , on = .(species_1, length_1 > high_lim) , len_flag := "length > 10sd from species mean in state" ]
wi_inland_fishobservations_10Nov2023[ , .N , len_flag]

rm(length_stats)

#summarize weight based stats
weight_stats <- wi_inland_fishobservations_10Nov2023[, .(stat = c(names(summary(weight_1)), "sd"),
...
#zero lengths
wi_inland_fishobservations_10Nov2023[weight_1 <= 0, .N]
wi_inland_fishobservations_10Nov2023[weight_1 == 0 , weight_flag := "weight reported as zero"]
...
wi_inland_fishobservations_10Nov2023[weight_stats , on = .(species_1, weight_1 > high_lim) , weight_flag := "length > 10sd from species mean in state" ]
wi_inland_fishobservations_10Nov2023[ , .N , weight_flag]

rm(weight_stats)

#summarize age based stats
age_stats <- wi_inland_fishobservations_10Nov2023[, .(stat = c(names(summary(age)), "sd"),
...
wi_inland_fishobservations_10Nov2023[age_stats , on = .(species_1, age > high_lim) , age_flag := "age > 10sd from species mean in state" ]
wi_inland_fishobservations_10Nov2023[ , .N , age_flag]


wi_inland_fishobservations_10Nov2023[, flag := do.call(paste, c(replace(.SD, is.na(.SD), ""), sep = "; ")), .SDcols= c("flag", "len_flag", "weight_flag", "age_flag")]
wi_inland_fishobservations_10Nov2023[, .N , flag ]
...
wi_inland_fishobservations_10Nov2023[ , c("len_flag", "weight_flag", "age_flag") := NULL , ]



#check speceis richness seems reasonable
wi_inland_fishobservations_10Nov2023[!is.na(total_effort_ident) , length(unique(species_1)) , total_effort_ident ][, hist(V1, main = "species richness per survey") , ]

#check fish counts:

wi_inland_fishobservations_10Nov2023[!is.na(species_1) & !is.na(total_effort_ident) , length(unique(obs_id)) , total_effort_ident ][, hist(V1, main = "N indiv fish sampled in survey") ,]

wi_inland_fishobservations_10Nov2023[!is.na(species_1) & !is.na(total_effort_ident) , length(unique(obs_id)) , .(total_effort_ident,lake_name,date_survey) ][V1>100000 , ,]

wi_inland_fishobservations_10Nov2023[total_effort_ident == 56232 , .N , .(species_1) ]
wi_inland_fishobservations_10Nov2023[total_effort_ident == 56232 & species_1 == "smallmouth_bass" , summary(length_1) ,  ]
wi_inland_fishobservations_10Nov2023[total_effort_ident == 56232 & species_1 == "smallmouth_bass" & is.na(length_1) , .N ,  ]
#flag those
wi_inland_fishobservations_10Nov2023[total_effort_ident == 56232 & species_1 == "smallmouth_bass" & is.na(length_1) , flag := ifelse(is.na(flag), "likely erroneous catch" , paste(flag, "likely erroneous catch", sep = ";" ))  ,  ]

wi_inland_fishobservations_10Nov2023[ , unique(flag)]

# Remove "; " from the beginning or end and compress multiples
wi_inland_fishobservations_10Nov2023[!is.na(flag), flag := trimws(gsub("^\\s*;+|;+\\s*$", "", gsub(";\\s*;+", "; ", flag)))]
wi_inland_fishobservations_10Nov2023[flag == "", flag := NA]

wi_inland_fishobservations_10Nov2023[!is.na(species_1) & !is.na(total_effort_ident) &is.na(flag) , length(unique(obs_id)) , total_effort_ident ][, hist(V1, main = "N indiv fish sampled in survey") ,]

wi_inland_fishobservations_10Nov2023[!is.na(species_1) & !is.na(total_effort_ident) , length(unique(obs_id)) , .(total_effort_ident,lake_name,date_survey) ][V1>20000 , ,]

#review other high catch records
wi_inland_fishobservations_10Nov2023[ total_effort_ident %in% 
                                        wi_inland_fishobservations_10Nov2023[!is.na(species_1) & !is.na(total_effort_ident) , length(unique(obs_id)) , .(total_effort_ident,lake_name,date_survey) ][V1>20000 , total_effort_ident, ],
...
wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(flag, "bad_date")) %>% 
  group_by(date_survey, date_total_effort_ident, date_sub_effort_ident, date_sample, year, month) %>% 
  count() %>% 
  print(n = nrow(.))

#year
wi_inland_fishobservations_10Nov2023[ , .N , is.na(year) ]
wi_inland_fishobservations_10Nov2023[ is.na(year), .N , is.na(date_sample) ]


  wi_inland_fishobservations_10Nov2023[ is.na(year) & !str_detect(flag, "bad_date"), year := lubridate::year(date_sample) ]

  
#month
wi_inland_fishobservations_10Nov2023[ , .N , is.na(month) ]
wi_inland_fishobservations_10Nov2023[ is.na(month), .N , is.na(date_sample) ]

  wi_inland_fishobservations_10Nov2023[ is.na(month) & !str_detect(flag, "bad_date"), month := lubridate::month(date_sample) ]
...
wi_inland_fishobservations_10Nov2023 %>% 
  ...
  count() %>% 
  print(n = nrow(.))
#nothing filled for bad dates, intentional 
...
wi_inland_fishobservations_10Nov2023[ ,notes_1 := trimws(notes_1) , ]
wi_inland_fishobservations_10Nov2023[notes_1 == "", .N]
wi_inland_fishobservations_10Nov2023[notes_1 == "" , notes_1 := NA]


#year to total effort ident year
wi_inland_fishobservations_10Nov2023 %>% 
  distinct(total_effort_ident, year, year_tei = lubridate::year(date_total_effort_ident)) %>% 
  mutate(check = year_tei - year) %>% 
  filter(check != 0)
#one total effort ident where the agency reported year does not match the tei date
check <- wi_inland_fishobservations_10Nov2023 %>% 
  filter(total_effort_ident == "67564") %>% 
  distinct(year, date_survey, date_total_effort_ident, date_sub_effort_ident, 
           date_sample, sampling_method_2, flag, total_effort_ident, sub_effort_ident, survey_type, 
           survey_id, total_effort_1, species_1, lake_name, lake_id)
glimpse(check)
rm(check)

wi_inland_fishobservations_10Nov2023 %>% 
  filter(survey_id == "79725") %>% 
  group_by(total_effort_ident, year, lake_name, sampling_method_2, date_sample, flag) %>% 
  count()
#looks like this stems from a big gap within the same survey_id 

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(year = case_when(is.na(year) ~ lubridate::year(date_total_effort_ident),
                          TRUE ~ year))
#now we should see all records within a total effort ident have a year, even if it differs from the survey date year

wi_inland_fishobservations_10Nov2023 %>% 
  filter(survey_id == "79725") %>% 
  group_by(total_effort_ident, year, lake_name, sampling_method_2, date_sample, flag) %>% 
  count()
#but it looks like we only solved the problem for when the year value was already NA, we will want to make sure that we take the total effort ident year for when total effort ident is not NA

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(year = case_when(!is.na(total_effort_ident) ~ lubridate::year(date_total_effort_ident),
                          TRUE ~ year)) 

wi_inland_fishobservations_10Nov2023 %>% 
  filter(survey_id == "79725" |
           str_detect(flag, "bad_date")) %>% 
  group_by(total_effort_ident, year, lake_name, sampling_method_2, date_sample, flag) %>% 
  count()
# looks good

#lets just take a look at any differences between survey years and total effort ident years
check <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(year_tei = lubridate::year(date_total_effort_ident),
         year_survey = lubridate::year(date_survey)) %>% 
  distinct(survey_id, total_effort_ident, year, year_tei, year_survey, date_survey, date_total_effort_ident) %>% 
  filter(year_tei != year_survey)
#cool, we can see here that when a survey spans years, we take the year from the total effort ident date but keep the survey date constant across survey records

# we do need to make date survey have the total effort ident date when it is na
wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(date_survey = case_when(is.na(survey_id) ~ date_total_effort_ident,
                                 TRUE ~ date_survey))
...
#fix the month so that it is the month of the tei when it is possible, otherwise take the sample month

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(month = case_when(!is.na(date_total_effort_ident) ~ lubridate::month(date_total_effort_ident),
                           TRUE ~ lubridate::month(date_sample)))

wi_inland_fishobservations_10Nov2023 %>% 
  filter(is.na(year)) %>% 
  summarise(tei_date = sum(!is.na(date_total_effort_ident)),
            sub_date = sum(!is.na(date_sub_effort_ident)),
            sample_date = sum(!is.na(date_sample)),
            total_na = sum(is.na(year)))
#now lets fill year and month in places where we have a sample date 

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(year = case_when(is.na(year) & !is.na(date_sub_effort_ident) ~
                            lubridate::year(date_sub_effort_ident),
                          is.na(year) & is.na(date_sub_effort_ident) & !is.na(date_sample) ~ 
                            lubridate::year(date_sample),
                          TRUE ~ year)) 



#add a flag for when there is more than one sampling method within a total effort ident 
wi_inland_fishobservations_10Nov2023 %>% 
  group_by(total_effort_ident, sampling_method_simple) %>% 
  summarise(n_sample = n_distinct(sampling_method),
            n_detailed = n_distinct(sampling_method_2)) %>% 
  filter(n_sample >1 |
           n_detailed >1) %>% 
  filter(!is.na(total_effort_ident)) %>% 
  print(n = nrow(.))
#we see here that some efishing surveys have multiple detailed sampling methods 
#lets make a flag for them

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  group_by(total_effort_ident, sampling_method_simple) %>% 
  mutate(method_flag = case_when((n_distinct(sampling_method) > 1 |
                                   n_distinct(sampling_method_2) > 1) & !is.na(total_effort_ident) ~ "multi types of efishing reported within tei",
                                 TRUE ~ NA))  %>% 
  ungroup() %>% 
  mutate(flag = case_when(
    !is.na(flag) & !is.na(method_flag) ~ paste(flag, method_flag, sep = ";"),
    is.na(flag) & !is.na(method_flag) ~ method_flag,
    TRUE ~ flag
  )) %>% 
  select(-method_flag)

# remove water clarity units when water clarity is not present 
wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(water_clarity_units = case_when(is.na(water_clarity) ~ NA,
                                         TRUE ~ water_clarity_units),
         water_temp_units = case_when(is.na(water_temp) ~ NA,
                                      TRUE ~ water_temp_units),
         survey_type = case_when(survey_type == "" ~ NA,
                                 TRUE ~ survey_type))

############add in cpue invalid column########################
...
wi_inland_fishobservations_10Nov2023 %>%
  mutate(effort_flag = case_when(str_detect(flag, "bad_date") ~ "y",
                                 str_detect(flag, "no date reported") ~ "y",
                                 str_detect(flag, "very high catch") ~ "y",
                                 str_detect(flag, "bad_date") ~ "y",
                                 str_detect(flag, "high effort") ~ "y",
                                 str_detect(flag, "duplicate") ~ "y",
                                 str_detect(flag, "umber of aged fish") ~ "y",
                                 str_detect(flag, "reported as 0") ~ "y",
                                 str_detect(flag, "multi types") ~ "y",
                                 TRUE ~ "n")) %>%
  group_by(effort_flag, flag) %>%
  count() %>%
  print(n = nrow(.))
...
wi_inland_fishobservations_10Nov2023 %>%
  group_by(total_effort_ident) %>%
  summarise(n = n(),
            n_efforts = sum(str_detect(flag, "bad_date") |
                              str_detect(flag, "no date reported") |
                                 str_detect(flag, "very high catch") |
                                 str_detect(flag, "bad_date") |
                                 str_detect(flag, "high effort") |
                                 str_detect(flag, "duplicate") |
                                 str_detect(flag, "umber of aged fish") |
                                 str_detect(flag, "reported as 0") |
                                 str_detect(flag, "multi types"), na.rm = T),
            flag_check = case_when(n_efforts == 0 ~ "none",
                                   n_efforts == n ~ "all",
                                   TRUE ~ "some"),
            .groups = "drop") %>%
  filter(flag_check == "some")
#there is a flag that denotes more aged fish than observed fish at the speciesXsurvey level - thus it isn't applied to all records, for our sake in creating this column lets just applied a flag to all records within the TEI. These total efforts were extensively look at and all contain the case where there were more aged fish than there were observed 

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>%
  group_by(total_effort_ident) %>%
  mutate(flag = case_when(
    any(str_detect(flag, "umber of aged fish"), na.rm = TRUE) ~ 
      paste(coalesce(flag, ""), "at least one species in TEI reported more aged fish than observed", sep = if_else(is.na(flag), "", ";")),
                          TRUE ~ flag)) %>%
  ungroup()
  ...
  summarise(n = n(),
            n_efforts = sum(str_detect(flag, "bad_date") |
                              str_detect(flag, "no date reported") |
                                 ...
                                 str_detect(flag, "high effort") |
                                 str_detect(flag, "duplicate") |
                                 str_detect(flag, "at least one species in TEI") |
                                 str_detect(flag, "reported as 0") |
                                 str_detect(flag, "multi types"), na.rm = T),
            flag_check = case_when(n_efforts == 0 ~ "none",
                                   n_efforts == n ~ "all",
            ...
            .groups = "drop") %>%
  filter(flag_check == "some")
#the only place where an effort flag is not applied to all in a total effort ident is na total efforts, this is good

wi_inland_fishobservations_10Nov2023 %>% 
  group_by(total_effort_ident) %>% 
  mutate(has_tei_flag = case_when(any(str_detect(flag, "umber of aged fish")) ~ "y",
                                  TRUE ~ "n")) %>% 
  ungroup() %>% 
  mutate(has_new_flag = case_when(str_detect(flag, "at least one species in TEI") ~ "y",
                                  TRUE ~ "n"),
         has_old_flag = case_when(str_detect(flag, "umber of aged fish") ~ "y",
                                  TRUE ~ "n")) %>% 
    group_by(has_tei_flag, has_new_flag, has_old_flag) %>% 
    count()
#we see here that not all records within a TEI had the original flag spite at a record within the TEI having it, there is a new flag generated at the TEI level 

#finally, the moment we have been waiting for
wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>%
  mutate(cpue_invalid = case_when(str_detect(flag, "bad_date") |
                                 str_detect(flag, "no date reported") |
                                 str_detect(flag, "very high catch") |
                                 str_detect(flag, "bad_date") |
                                 str_detect(flag, "high effort") |
...
wi_inland_fishobservations_10Nov2023 %>% 
  group_by(cpue_invalid) %>% 
  count()

wi_inland_fishobservations_10Nov2023 %>% 
  filter(is.na(date_total_effort_ident) & !is.na(total_effort_ident)) %>% 
  group_by(flag) %>% 
  count()

####updating aging structure####
wi_inland_fishobservations_10Nov2023 %>% 
  distinct(aging_structure_1)

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(aging_structure_1 = case_when(aging_structure_1 == "none" ~ NA,
                                       TRUE ~ aging_structure_1))

wi_inland_fishobservations_10Nov2023 <- wi_inland_fishobservations_10Nov2023 %>% 
  mutate(aging_structure_1 = case_when(is.na(age) ~ NA,
                                       TRUE ~ aging_structure_1))
```
...
wi_inland_fishobservations_10Nov2023 %>% 
  filter(species_1 %in% c("walleye",
                          ...
                          "smallmouth_bass",
                          "yellow_perch")) %>% 
  summarise(problem = sum(!is.na(length_1) & is.na(est_age)))
#this is the number of fish we filtered out due to not making the ALK length cut
...
fwrite(wi_inland_fishobservations_10Nov2023, "Data_and_Scripts/Data/output/wi_data.csv" , na="NA" )

#remove the object
rm(wi_inland_fishobservations_10Nov2023)

#read in the object
wi_dat <- fread("Data_and_Scripts/Data/output/wi_data.csv", na.strings = "NA")
glimpse(wi_dat)

#make sure column are correct and in the correct form
wi_dat <- wi_dat %>% 
  mutate(
...
write_dataset(dataset = wi_dat, 
              path = "Data_and_Scripts/Data/output/wi_file_arrow", 
              format = "parquet")
...
# wi_inland_fishobservations_10Nov2023 <- as_tibble(wi_inland_fishobservations_10Nov2023)
    ...
    # 
    # arrow::write_dataset(dataset = wi_dat, path = "Data_and_Scripts/Data/output/wi_file_arrow", format = "parquet")
    # 
    # wi_data <- open_dataset(file = "Data_and_Scripts/Data/output/wi_file_arrow" )
    # 
    # glimpse(wi_data)
```
