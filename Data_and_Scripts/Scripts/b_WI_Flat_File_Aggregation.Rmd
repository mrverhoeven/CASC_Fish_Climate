...
Patched  16. Why are no cisco showing up in WI?
      ...
      26. The total effort idents restructure now results in bad splitting over the targeted species column. Add targeted species info to the TEI assignment.
...
cisco <- fread("Data_and_scripts/Data/input/wi_target_cisco8nov2023.csv")


cisco[ , ,  ]

wi_inland_fishobservations_19Mar2021[ , date := as.IDate(date) ,]

colnames(cisco)
setnames(cisco,
         old = c("sampling_method_1", "species_1", "total_effort_1_units", "nothing_caught"), 
         new = c("sampling_method", "species", "total_effort_1_unit", "total_effort_nothing_caught"))
wi_inland_fishobservations_19Mar2021 <- rbindlist(list(cisco, wi_inland_fishobservations_19Mar2021), use.names = TRUE, fill = TRUE)

wi_inland_fishobservations_19Mar2021[ , state := "Wisconsin" , ]
...
```{r}
glimpse(wi_inland_fishobservations_19Mar2021)
...
wi_data <- wi_inland_fishobservations_19Mar2021 %>% 
  mutate(county = case_when(is.na(county) ~ tolower(County),
                            TRUE ~ county),
