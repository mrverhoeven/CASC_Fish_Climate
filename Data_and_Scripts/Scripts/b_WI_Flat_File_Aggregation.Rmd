...
####### note we are going to ask for WI to send us individual fish data WITH ages week of 7/10/23
...
wi_inland_effort_19Mar2021[ , , survey_id] # Holly E said to be cautious about survey ID
    # yes!
...
wi_inland_effort_19Mar2021[ , , .(lake_id, year, sampling_method)]
...
wi_inland_effort <- wi_inland_effort_19Mar2021 %>%
  ...
  relocate(county, lake_name.1, lake_id, year, survey_id, sample_id.1, sampling_method, simple_gear.x, date.1, distance, distance_units, sample_time_notes.1, gear_data_notes.9, sample_time_notes.3)%>% #take these columns and put them in front
...
winnebago_cpue_comp <- winnebago_calc_cpue %>%
  full_join(wi_winnebago_cpue_23Apr2021, by = c("site_id.1", "site_id.2", "site_id.3", "site_id.4", "species.1", "date.1", "gear_data_notes.1"))%>%
...
Finalize Winnebago file so individual fish observations have an effort and effort units column
```{r}
winnebago_fish_effort <- wi_winnebago_fishobservations_23April2021 %>%
  mutate(effort = "1")%>%
  mutate(effort_units = "5 minute trawl")%>%
  select(year, date.1, lake_name.1, lake_id, site_id.1, site_id.2, site_id.3, site_id.4, gear_data_notes.1, sampling_method, species.1, total_count, length.1, young_of_year, total_catch, effort, effort_units, jan2023_inventoried, file_number, file_status, state, source_contact, date_recieved, source_agency, file_path, new_file_name, file_type, data_type, targeted_or_standard, individual_count, original_file_name.1, unique_row_key.1) #re-order columns

#saveRDS(winnebago_fish_effort, file = "Data_and_Scripts\\Data\\output\\wi_winnebago_indiv_fish_w_effort.rds")
```
Winnebago is DONE

Investigate inland_fishobservations file
```{r}
wi_inland_fishobservations_19Mar2021[!is.na(age), .N] #no fish in this file have ages... 

wi_inland_fishobservations_19Mar2021[!is.na(length.1), .N] #7 million have lengths
...
```{r}
fish_ob_test <- wi_inland_fishobservations_19Mar2021[survey_id == "281" & sampling_method == "fyke_net" & species.1 == "walleye"][,.(survey_id, sampling_method, species.1, lake_id, length.1, date.1),]
# 650 walleye in a fyke net in this survey


len_age_test <- wi_inland_lenage_19Mar2021[survey_id == "281" & sampling_method == "fyke_net" & species.1 == "walleye"]
...
# eliminate instances where age is NA in length age file and try a similar test as above
wi_inland_lenage_noNAage <- wi_inland_lenage_19Mar2021[!is.na(age)] #443811


len_age_test2 <- wi_inland_lenage_19Mar2021[survey_id == "400955914" & sampling_method == "fyke_net" & species.1 == "walleye"][,date.1:= as.IDate(date.1)] #55

fish_ob_test2 <- wi_inland_fishobservations_19Mar2021[survey_id == "400955914" & sampling_method == "fyke_net" & species.1 == "walleye"] #88 

overlap_check <- fish_ob_test2 %>%
  inner_join(len_age_test2, by = c("survey_id", "sampling_method", "species.1", "length.1", "date.1", "lake_id"))

len_age_test2[, .N, length.1]

fish_ob_test2[, .N, length.1]

fish_ob_plot2 <- ggplot(data = fish_ob_test2, aes(x = length.1))+geom_histogram(binwidth = .1)
len_age_plot2 <- ggplot(data = len_age_test2, aes(x = length.1))+geom_histogram(binwidth = .1)

compare_plot2 <- plot_grid(fish_ob_plot2, len_age_plot2, labels = c("fish observations", "length age"))
# again, instances of fish in length age that are NOT in fish obs
...
wi_inland_effort[ , date.1 := as.IDate(date.1,format = "%m/%d/%Y"),]

WI_indiv_fish_w_effort <- merge(wi_inland_fishobservations_19Mar2021, wi_inland_effort, by = c("lake_id", "date.1", "sampling_method", "sample_id.1"), suffixes = c("_indivfish", "_effort"), all = T)
...
```{r}
#prep for merge
winnebago_fish_effort[ , date.1 := as.IDate(date.1,format = "%Y/%m/%d"),]


wi_inland_and_winnebago <- rbindlist(list(WI_indiv_fish_w_effort, winnebago_fish_effort),
                               fill = TRUE,
                               use.names = TRUE)
```


Cast wide to make species matrix and add instances of 0 catch
```{r}
wi_individ_fish_with_effort_WIDE <- dcast(WI_indiv_fish_w_effort, ... ~ species.1 , value.var = "total_count", fill = 0)
```
