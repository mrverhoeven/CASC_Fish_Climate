...
wi_inland_fishobservations_10Nov2023[ , .N , year ]
...
# so a quick recap here is that there are ~20k records in the fish obs that do not have a sample date a person could dredge up the survey start and end dates, but it is not clear to me how that could help ID fish within a survey
...
#add identifiers for individual fish-lengths, weights, and sex, structure in a survey 
...
#check work -- not working
test[  , .N  , .("Has_Observatinon_Dat" = !is.na(original_file_name_indivfish), "Has_age_dat" = !is.na(age_agedfish))   ]
test[  , .N , .() ]
...
wi_inland_fishobservations_10Nov2023[ , .N , .(is.na(date), is.na(year)) ] #no cases of na date where year can cover, drop year
  wi_inland_fishobservations_10Nov2023[is.na(date)& is.na(year)]
...
wi_target_cisco8nov2023 <- read_csv("E:/Shared drives/Hansen Lab/RESEARCH PROJECTS/Fish Survey Data/WI_Data/wi_raw_disaggregated_data/wi_target_cisco8nov2023.csv")
...
wi_inland_fishobservations_10Nov2023[ file ==2 , .N , original_file_name ]
  wi_inland_fishobservations_10Nov2023[ file ==2 , original_file_name := "wdnr_inland_cpue_data.csv" ]
