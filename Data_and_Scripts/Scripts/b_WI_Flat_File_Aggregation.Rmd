...
# Preamble
6/25 - I have added code to explore the effort values associated with net samples. I have also added some code to calculate night nights from hours and number of nets within line 423. Now I need to apply the net night values to places where it should be. I will need to explore the "simple gears" of netting to ensure all of those should get that net nights (do seines make it in here). For places that have both number of nights and hours - flag?

There is another issue where effort is based on the targeted species. An example would be Chippewa in 1990 fyke netting wehre walleye get all of the effort and then game fish are ten less due to not being included as a target species in ten of the nets. In our current total effort grouping framework, I am almost certain that we will split out those other 10 nets as their own total effort ident. I need to double check what is in our current total effort ident grouping - if it contains target species - make sure it contains secondary target species as well and see how they are being split out. How often is there multiple target species within a survey id (or in our case total effort ident)
