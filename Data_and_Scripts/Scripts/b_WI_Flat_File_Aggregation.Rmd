...
# this notes is made on 6/26/25 - below had previously converted number of nets to gill nets. I have since vetted the total effort and calcuated night nets in the effort prep chunk. I still need to confirm that these efforts were drawn from lines 967-977 pulling number of nets in places where total effort 1 and 2 were both absent. If that is true, we cannot just convert to net_nights becuase we can't assume the duration that they were sent 
...
#also below is code that creates a sample id based on date and sampling method - that doesn't appear to be correat either... I think each site is more representative of a sample_id - lets add that to the grouping
#when looking below this is totally the case - total effort columns are transitioned into sub effort columns and sample_id gets shifted to sub effort ident. The old groupin of just date in winne makes it look like there is just a total effort of 5 for a given day when it is really 15*5. Thus, lets include site id as the sample id for these 
...
wi_inland_fishobservations_10Nov2023 %>% 
  filter(str_detect(lake_name, "innebag") & date == c("2014-08-06", "2014-08-07")) %>% 
...
wi_inland_fishobservations_10Nov2023 %>% 
  ...
  mutate(has_new_flag = case_when(str_detect(flag, "at least one species in TEI") ~ "y",
    ...
    count()
#we see here that not all records within a TEI had the original flag dispite at a record within the TEI having it, there is a new flag generated at the TEI level 
...
write_dataset(dataset = wi_dat, path = file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Fish Survey Data", "Parquet files", "hive_update", "state=Wisconsin"), format = "parquet")
...
mw_data %>% 
  filter(state == "Wisconsin") %>% 
  group_by(notes_1) %>% count() %>% 
  collect()

mw_data %>% 
   #filter(state == "Wisconsin") %>% 
   group_by(state, length_unit_1) %>% count() %>% 
   collect()

wi_dat %>% 
  filter(lake_name == "lake_chippewa" & year == 1990 & sampling_method == "fyke_net") %>% 
...
wi_dat %>% 
  filter(str_detect(lake_name, "innebag") & date_total_effort_ident %in% c("2014-08-07", "2014-08-06")) %>% 
  group_by(total_effort_ident, sub_effort_ident, site_id, total_effort_1, total_effort_1_units, sub_effort_1, sub_effort_1_units) %>% 
  summarise(n_walleye_sub = sum(species_1 == "walleye")) %>%
  group_by(total_effort_ident) %>% 
  mutate(n_walleye_total = sum(n_walleye_sub)) %>% 
  collect() %>% 
  print(n = nrow(.))
```
