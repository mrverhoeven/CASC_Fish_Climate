...
quick effort check to see when these surveys took place
```{r}
wi_cisco_check <- wi_cisco_clean%>%
  mutate(date = mdy(date))%>%
  mutate(month = month(date))%>%
  filter(month > 8)%>%
  filter(total_count > 0) #probably want to extend filter window for VGN to go through September
```
