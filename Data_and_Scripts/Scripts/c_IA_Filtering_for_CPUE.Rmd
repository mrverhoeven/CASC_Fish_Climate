...
#data
```{r}
ia_data <- open_dataset(sources = file.path("D:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Fish Survey Data", "Parquet files", "hive_update"), partitioning = c("state")) %>% 
  filter(state == "Iowa")
glimpse(ia_data)
```
...
ia_data %>% 
  ...
  print(n = nrow(.))
```
Hours is the standard Iowan effort unit - we need to make sure only do not use distance. There appear to be some NA unit values, all of those have an adequacy flag tagged on them and it is not nessary to worry about filtering on that.
...
good_surveys <- ia_data %>% 
  ...
  #creating a y or n for if there is flag for the effort associated with the catch
  mutate(species_included = case_when(str_detect(target_species, "walleye") | is.na(target_species) ~ "y",
                                      TRUE ~ "n"),
...
good_surveys %>% 
  group_by(survey_type_filter, species_included, effort_flag) %>% 
  count() 
...
adult_walleye %>% 
  distinct(total_effort_ident,target_species, species_included,survey_type, survey_type_filter, effort_flag) %>%
  group_by(target_species, species_included,survey_type, survey_type_filter, effort_flag) %>% 
   collect() %>% 
   count() 
