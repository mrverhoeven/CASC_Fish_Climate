...
in_data %>% 
  ...
  collect()
```
Looks like we have some effort flags, some noting missing effort and some noting high effort. We will not have to worry about the missing effort as it will get weeded out naturally, but we will have to filter out the high effort case. We will create a y/n column that notes if there is an effort flag for filtering or not and then filter on this - similar to what we do with targeted species. 
...
good_surveys <- in_data %>% 
  ...
  mutate(species_included = case_when(grepl("walleye", target_species) | is.na(target_species) ~ "y",
                                      TRUE ~ "n"),
         effort_flag = case_when(grepl("high effort", flag) ~ "y",
                          TRUE ~ "n")) %>% 
