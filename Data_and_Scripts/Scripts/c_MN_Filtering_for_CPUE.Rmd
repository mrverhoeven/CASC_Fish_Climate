...
mn_data %>% 
  ...
  summarise(less.1 = sum(total_effort_1 <= 1),
            ...
            before.june = sum(month(date) < 6),
            before2020 = sum(year(date) < 2020),
            n = n()) %>% 
...
#13 surveys that have more than one total effort (removes 26 records)
...
adult_walleye_cpue %>% 
  ...
  group_by(lake_name, survey_type, sampling_method, total_effort_1, date, total_effort_nothing_caught, cpue) %>% 
  count() %>% 
  print(n = nrow(.))
