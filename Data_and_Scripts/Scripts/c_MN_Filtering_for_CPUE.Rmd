...
In Minnesota, we are only interested in the standard surveys, passive gears where species are not "targeted" so we will not worry about the target species column
...
#data filters for MN
working with mn data directly to select sampling methods, survey types, area filters, etc.
```{r}
#1.) picking the standard sampling methods
mn_data %>% 
...
mn_data %>% 
  ...
  count() %>% 
  collect()
#we see here that the most used and standard trap net is the 0.75 inch doubel frame
...
#3.) lake area - first we need to grab the lake areas
mn_data %>% 
  left_join(area) %>% 
