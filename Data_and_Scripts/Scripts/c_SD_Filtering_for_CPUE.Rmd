...
sd_data %>% 
  ...
  collect() %>% 
  print(n = nrow(.))
#looks like nets are in net night and efishing is in mintues
```
...
good_surveys <- sd_data %>% 
  right_join(filter_table_effort, by = c("state", "sampling_method")) %>% 
  group_by(lake_name_1, total_effort_ident) %>% 
  arrange(date_1) %>%
  ungroup() %>% 
  collect() 
  slice(1) %>%  
...
#problem with na counts
check <- adult_walleye %>% 
  filter(total_effort_ident == 4)
#looks like I am having trouble when a total effort ident caught something but it has sub efforts that caught nothing
#I can fix it by adding nothing caught in the grouping but then it reports multipule cpues for a given total effort ident
```
