...
wi_data %>% 
  ...
  print(n = nrow(.))
```
Wisconsin is heavy on the targeted sampling, so we need to be savvy in picking the surveys in which would be valid for abundance measures. See the example above that will be applied to in various forms to all species filtering below. This code creates a yes or no if we should keep that survey for a cpe calculation based on if the species was targeted. We have to be a little creative with the string detection because we want to collect "gamefish" and "gamefish_species" but we DONT want to collect "non-gamefish" or panfish_gamefish" - if searching for walleye. Note that we are more picky here, we do not include if the the survey is NA for targeted species - a Wisconsin type of thing were we cannot just assume that a survey would be good to measure walleye cpe if it were not stated 
...
good_surveys %>% 
  group_by(total_effort_ident, water_temp, sampling_method) %>% 
  count()
#duplicate total effort idents are attributed to multiple water temps within a total effort ident and multiple sampling methods within a total effort ident - SEE NOTES on Planning DOC
...
adult_bluegill %>% 
  ...
  filter(n_gears >1)
#this shows 25 total effort idents where there are more than one sampling method listed within a total effort ident
...
good_surveys %>% 
  distinct(total_effort_ident, water_temp, sampling_method) %>% 
  count()
...
#old code to return to for aggregation##
wi_data %>% 
  filter(total_effort_ident %in% check$total_effort_ident) %>% 
  distinct(total_effort_ident, sampling_method_simple, sampling_method, sampling_method_2) %>% 
  collect() %>% 
  print(n = nrow(.))
#this shows several places where there are more than one sampling method within a total effort ident

wi_data %>% 
  filter(total_effort_ident %in% check$total_effort_ident) %>% 
  distinct(total_effort_ident, sampling_method_simple, sampling_method, sampling_method_2) %>% 
  collect() %>% 
  filter(!str_detect(sampling_method_simple, "electro")) %>% 
  print(n = nrow(.))
#all of them are electrofishing surveys 
rm(check)
...
wi_data %>% 
  filter(!is.na(total_effort_ident)) %>% 
  group_by(total_effort_ident) %>% 
  summarise(n_gears = n_distinct(sampling_method_2)) %>% 
  filter(n_gears >1) %>% 
  collect()
#it is only a problem at the sampling method and sampling method 2 level
