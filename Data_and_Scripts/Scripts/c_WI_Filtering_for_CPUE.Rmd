...
wi_data %>% 
  ...
  print(n = nrow(.))
#it looks like for any given gear there could be multipule types of effort reported
...
wi_data %>% 
  filter(sampling_method %in% c("fyke_net") | str_detect(sampling_method, "boat_electrofishing")) %>% 
  mutate(units_flag = case_when(sampling_method == "fyke_net" & total_effort_1_units == "hours" ~ "y",
  ...
  collect()
```
For fyke nets it is really obvious that it is standard to report in hours and we will proceed only using hours for that gear. For electrofishing the most used unit is minutes, we will need to make sure that only minutes show up for electrofishing and only hours for fykes.
...
good_surveys %>% 
  distinct(total_effort_ident, water_temp) %>% 
  count()
#all attributed to water temp and sampling method
