...
# Data
```{r}
wi_data <- open_dataset(sources = file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Fish Survey Data", "Parquet files", "hive_update"), partitioning = c("state")) %>% 
...
wi_data %>% 
  ...
  print(n = nrow(.))
```
Wisconsin is heavy on the targeted sampling, so we need to be savvy in picking the surveys in which would be valid for abundance measures. See the example above that will be applied to in various forms to all species filtering below. This code creates a yes or no if we should keep that survey for a cpe calculation based on if the species was targeted. We have to be a little creative with the string detection because we want to collect "gamefish" and "gamefish_species" but we DONT want to collect "non-gamefish" - if searching for walleye. Note that we are more picky here, we do not include if the the survey is NA for targeted species - a Wisconsin type of thing were we cannot just assume that a survey would be good to measure walleye cpe if it were not stated 
...
wi_data %>% 
  ...
  collect()
```
For fyke nets it is really obvious that it is standard to report in net_nights and we will proceed only using hours for that gear. For electrofishing the most used unit is minutes, we will need to make sure that only minutes show up for electrofishing and only hours for fykes.
...
good_surveys <- wi_data %>%
  mutate(targeted_in_survey = case_when(str_detect(target_species, "walleye") ~ "y",
                                        ...
                                        str_detect(target_species, "escocid_percid") ~ "y",
                                        TRUE ~ "n"),
         effort_flag = case_when(str_detect(flag, "bad_date") ~ "y",
                                 str_detect(flag, "very high catch") ~ "y",
                                 str_detect(flag, "bad_date") ~ "y",
                                 str_detect(flag, "high effort") ~ "y",
                                 str_detect(flag, "duplicate") ~ "y",
