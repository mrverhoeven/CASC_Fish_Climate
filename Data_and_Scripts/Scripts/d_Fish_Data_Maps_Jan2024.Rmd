...
# WI all data
```{r}
wi_data <- open_dataset("G:/Shared drives/Hansen Lab/RESEARCH PROJECTS/Fish Survey Data/Parquet files/wi_file_arrow")
...
CC_Dat <- read_csv("G:/Shared drives/Hansen Lab/RESEARCH PROJECTS/Fish Survey Data/Data for Collaborators/Preliminary_CPUE_Data/all_state_cpue_8Dec23.csv")
```
...
NOP_CPUE_2010_map <- ggplot()+
  ...
  coord_fixed(1.3)+
  guides(fill = FALSE)+
  geom_point(data = NOP_CPUE_2010, aes(x = lon, y = lat, color = Ave_CPUE_NOP_2010), alpha = 0.5)+
  theme_void()+
  scale_color_viridis(option = "H")+
  labs(title = "Average CPUE for Northern Pike, 2010 - 2023", color = "average CPUE", alpha = "average CPUE")
