...
mw_data %>% 
  summarise(min = as.character(min(latitude_lake_centroid, na.rm = T)),
            max = as.character(max(latitude_lake_centroid, na.rm = T))) %>% 
  collect()
...
mw_data %>% 
  summarise(min = as.character(min(longitude_lake_centroid, na.rm = T)),
            max = as.character(max(longitude_lake_centroid, na.rm = T))) %>% 
  collect()
...
#so our earliest year is 1940 based off total effort ident but we have sub and sample dates prior?
mw_data %>% 
  filter(year(date_sub_effort_ident) < 1940) %>% 
  group_by(is.na(total_effort_ident), state) %>% 
  count() %>% 
  collect()

mw_data %>% 
  filter(year(date_sample) < 1940) %>% 
  group_by(is.na(total_effort_ident), state) %>% 
  count() %>% 
  collect()
...
#aging structure 
mw_data %>% 
  distinct(state, aging_structure_1) %>% 
  collect() %>% 
  arrange(aging_structure_1) %>% 
  print(n = nrow(.))
...
mw_data %>% 
  distinct(state, lake_name, lake_id, nhdhr_id, latitude_lake_centroid) %>% 
  group_by(state) %>%
